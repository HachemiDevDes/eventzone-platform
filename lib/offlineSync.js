...
import { toggleAttendeeCheckin, upsertAttendee, permanentDeleteAttendee, archiveAttendee, restoreAttendee, broadcastRealtimeChange } from "./db";
  ...
  try {
    for (let i = 0; i < queue.length; i++) {
      ...
      try {
              ...
              if (raw) {
                ...
                if (Array.isArray(list)) {
                  ...
                  if (idx !== -1) {
                    ...
                    window.dispatchEvent(new CustomEvent("eventzone:checkin-synced", {
                      detail: { eventId: targetEvId, attendeeId: targetAttId, checkedIn: isChecked, list }
                    }));
                  }
                }
              }
          ...
          if (typeof window !== "undefined" && savedAttendee) {
            try {
              ...
              const idx = list.findIndex((a) =>
                (targetOldId && a.id === targetOldId) ||
                (savedAttendee.id && a.id === savedAttendee.id) ||
                (a.email && savedAttendee.email && a.email.toLowerCase() === savedAttendee.email.toLowerCase())
              );
              const cleanRecord = { ...savedAttendee, _syncFailed: false };
              if (idx !== -1) {
                list[idx] = { ...list[idx], ...cleanRecord };
              } else {
                list.unshift(cleanRecord);
              }
              localStorage.setItem(cacheKey, JSON.stringify(list));

              window.dispatchEvent(new CustomEvent("eventzone:attendee-synced", {
                detail: { eventId: targetEvId, attendee: cleanRecord, list }
              }));
            } catch (cacheErr) {
            ...
            try {
              ...
              if (raw) {
                ...
                if (Array.isArray(list)) {
                  ...
                  window.dispatchEvent(new CustomEvent("eventzone:checkin-synced", {
                    detail: { eventId: targetEvId, attendeeIds, checkedIn: isChecked, list: updatedList }
                  }));
                }
              }
            } catch (cErr) {
        ...
        } else if (action.type === "restore_attendee") {
          const targetAttId = action.attendeeId || action.payload?.id;
          const targetEmail = action.payload?.email;
          const targetEvId = action.eventId || eventId;
          await restoreAttendee(targetAttId, targetEmail, targetEvId);
          if (typeof window !== "undefined") {
            try {
              const cacheKey = `eventzone_cache_attendees_${targetEvId}`;
              const raw = localStorage.getItem(cacheKey);
              ...
              console.warn("Cache update after restore_attendee error:", cErr);
            }
          }
        }
...
export function useOfflineSync(eventId) {
  ...
  useEffect(() => {
    ...
    const handleOnline = async () => {
      ...
      if (verified) {
        // Do NOT call processAllOfflineQueues() here — loadEventData's
        // online handler already does it. Calling it here races and causes
        // SYNC_IN_PROGRESS contention + stale data overwrites.
        // Just refresh queue state and let syncNow handle any stragglers.
        refreshQueue();
        setSyncState("online");
      } else {
        setSyncState("offline");
    ...
    checkNetworkConnectivity().then((online) => {
      setIsOnline(online);
      if (online) {
        // Don't call processAllOfflineQueues here — loadEventData handles it.
        // Just sync this event's queue if needed.
  ...
  const queueDeleteAttendee = useCallback(
    (attendeeId, email = null) => {
      ...
      if (typeof window !== "undefined") {
        try {
          const cacheKey = `eventzone_cache_attendees_${eventId}`;
          const raw = localStorage.getItem(cacheKey);
          if (raw) {
            const list = JSON.parse(raw);
            if (Array.isArray(list)) {
              const filtered = list.filter(a => a.id !== attendeeId && (!email || !a.email || a.email.toLowerCase() !== email.toLowerCase()));
              localStorage.setItem(cacheKey, JSON.stringify(filtered));
            }
          }
        } catch (e) {}
      ...
      if (typeof window !== "undefined") {
        try {
          ...
          if (raw) {
            ...
            if (Array.isArray(list)) {
              const updated = list.map(a => {
                if (a.id === attendeeId || (email && a.email && a.email.toLowerCase() === email.toLowerCase())) {
                  return { ...a, status: "archived", status_participation: "archived", isArchived: true };
                }
                return a;
              });
  ...
  // Helper to queue restore attendee action
  const queueRestoreAttendee = useCallback(
    (attendeeId, email = null) => {
      if (!eventId || !attendeeId) return;
      enqueueOfflineAction(eventId, {
        type: "restore_attendee",
        attendeeId,
        payload: { id: attendeeId, email },
      });
