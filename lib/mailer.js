...
export async function sendExhibitorPacketEmail({
  ...
  const defaultSubject = isSponsor
    ? `Sponsor Floor Plan & Venue Packet: ${name} - ${finalEventTitle}`
    : `Exhibitor Booth Information: ${name} - ${finalEventTitle}`;
  const finalSubject = subject || defaultSubject;

  const headerTitle = isSponsor
    ? "Sponsor Floor Plan & Venue Packet"
    : "Exhibitor Booth & Floor Plan Packet";
  ...
  const content = `
    ...
    <div class="card">
      <div class="card-header">${isSponsor ? "Partner Space & Sponsorship Allocation" : "Exhibitor Space Allocation"}</div>
      <table role="presentation" class="info-table" cellpadding="0" cellspacing="0" border="0">
        <tr>
  ...
  return await sendEmail({
    ...
    attachments,
    html: getEmailLayout({
      title: `${isSponsor ? "Sponsor Packet" : "Exhibitor Packet"} - ${name}`,
      preheader,
      content,
