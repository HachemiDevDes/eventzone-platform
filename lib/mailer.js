...
export async function sendTeamInviteEmail({
  ...
  const finalInviteUrl = inviteUrl || (process.env.NEXT_PUBLIC_APP_URL ? `${process.env.NEXT_PUBLIC_APP_URL}/?eventId=${eventId || ''}` : `https://eventzone.pro/?eventId=${eventId || ''}`);
  ...
  const content = `
    ...
    <div style="text-align: center; margin: 32px 0 28px 0;">
      <a href="${finalInviteUrl}" target="_blank" rel="noopener noreferrer" style="display: inline-block; background-color: #2563eb; color: #ffffff; text-decoration: none; padding: 14px 32px; border-radius: 12px; font-weight: 800; font-size: 14px; letter-spacing: -0.2px; box-shadow: 0 4px 14px rgba(37, 99, 235, 0.35); transition: background-color 0.2s;">
        Accept Invitation & Access Workspace &rarr;
      </a>
      <p style="font-size: 12px; color: #94a3b8; margin-top: 12px;">
        Clicking this link will direct you securely to the event management dashboard.
      </p>
    </div>
