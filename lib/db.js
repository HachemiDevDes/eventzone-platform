...
function mapSponsorToDb(sponsor, eventId = _activeEventId) {
  ...
  return {
    ...
    image_url: sponsor.image || sponsor.logo,
    org_id: targetOrgId,
  ...
  try {
    ...
    if (eventId && isValidUuid(eventId)) {
      ...
      if (!evErr && ev) {
        ...
        if (evPass && evPass === cleanPasscode) {
          matchedEvent = mapEventFromDb(ev);
        }
      }
    }

    // 2. If no event matched yet, search events by exact checkin_passcode
    if (!matchedEvent) {
      const { data: events, error } = await supabase
        .from('events')
        .select('*')
        .not('checkin_passcode', 'is', null)
        .order('created_at', { ascending: false });
    ...
    // 3. Resolve staff identity: Check if email belongs to event team or event organizer
    let teamMember = null;
    try {
      ...
      if (teamRow) {
        teamMember = {
          id: teamRow.id,
          name: teamRow.name || `${teamRow.first_name || ''} ${teamRow.last_name || ''}`.trim() || 'Staff',
          email: teamRow.email,
          role: teamRow.role || 'Check-in Staff',
          department: teamRow.department || 'Operations',
    ...
    // Also allow organizer account email
    if (!teamMember) {
      ...
      if (organizerProfile && (matchedEvent.organizer_id === organizerProfile.id || matchedEvent.organizerId === organizerProfile.id)) {
        teamMember = {
...
export async function upsertCertificateTemplate(template, eventId = _activeEventId) {
  const targetId = eventId || _activeEventId;
  const tplId = template.id || generateUuid();
  const formatted = {
    ...template,
    id: tplId,
    eventId: targetId,
