...
export async function fetchEventDetails(eventId = _activeEventId) {
  ...
  try {
    ...
    if (isUuid) {
      query = query.or(`id.eq.${targetId},slug.eq.${targetId}`);
    } else {
...
function mapEventFromDb(row) {
  ...
  return {
    ...
    checkinPasscode: row.checkin_passcode || (row.id ? String(row.id).slice(0, 6).toUpperCase() : '202688'),
    checkin_passcode: row.checkin_passcode || (row.id ? String(row.id).slice(0, 6).toUpperCase() : '202688'),
    gallery: parsedGallery,
...
function mapEventToDb(details) {
  ...
  if (details.portalSettings !== undefined || details.portal_settings !== undefined) {
    payload.portal_settings = details.portalSettings || details.portal_settings || { networking: true, agenda: true, exhibitors: true, sponsors: true, floorplans: true, resources: true, announcements: true };
  }
...
export async function permanentDeleteAttendee(id) {
  if (!id || !isValidUuid(id)) return;
  try {
    const { error } = await supabase.from('participants').delete().eq('id', id);
    if (error) console.warn("permanentDeleteAttendee notice:", error.message);
  } catch (e) {
...
export async function upsertOrganization(org, eventId = _activeEventId) {
  const targetId = eventId || _activeEventId;
  const row = mapOrgToDb(org, targetId);
  try {
    const { data, error } = await supabase
      .from('organizations')
      .upsert(row, { onConflict: 'id' })
    ...
    return { ...mapOrgFromDb(data), ...org, eventId: targetId };
  } catch (e) {
...
function mapOrgToDb(org, eventId = _activeEventId) {
  ...
  const targetEventId = eventId || org.eventId || _activeEventId;
  return {
    id: validId,
...
export async function upsertSponsor(sponsor, eventId = _activeEventId) {
  ...
  try {
    ...
    return { ...mapSponsorFromDb(data), ...sponsor };
  } catch (e) {
...
function mapSponsorToDb(sponsor, eventId = _activeEventId) {
  ...
  return {
    ...
    org_id: isValidUuid(sponsor.orgId) ? sponsor.orgId : (isValidUuid(sponsor.org_id) ? sponsor.org_id : null),
    amount: sponsor.amount !== undefined && sponsor.amount !== null && sponsor.amount !== '' ? parseFloat(sponsor.amount) : null,
    currency: sponsor.currency || 'DZD',
    booth: sponsor.booth || '',
    perks: Array.isArray(sponsor.perks) ? sponsor.perks : [],
    notes: sponsor.notes || '',
...
function mapExhibitorToDb(exhibitor, eventId = _activeEventId) {
  ...
  return {
    ...
    staff_count: exhibitor.staffCount || 2,
    ...
    logo_url: exhibitor.logo || exhibitor.logo_url || '',
...
export const STARTER_DOCUMENTS = [
  {
    id: "doc-starter-1",
    eventId: DEFAULT_EVENT_ID,
    name: "CIC Venue Lease & Safety Agreement 2026",
    fileName: "CIC_Venue_Lease_Agreement_2026.pdf",
    ...
    accessLevel: "team",
    description: "Official venue contract with Algiers International Conference Center (CIC), security annexes, and operational clauses.",
    uploadedBy: "Event Admin",
    isPinned: true,
    isArchived: false,
  ...
  },
  {
    id: "doc-starter-2",
    eventId: DEFAULT_EVENT_ID,
    name: "Official Sponsorship Prospectus & Media Kit",
    fileName: "Algeria_Hydrogen_Sponsorship_Prospectus_2026.pdf",
    fileUrl: "https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf",
    fileSize: 4404019, // 4.2 MB
    fileType: "pdf",
    mimeType: "application/pdf",
    category: "Sponsorship & Media",
    accessLevel: "sponsors",
    description: "Comprehensive sponsorship tiers (Platinum, Gold, Silver), booth dimensions, brand exposure metrics, and VIP access packages.",
    uploadedBy: "Partnerships Lead",
    isPinned: true,
    isArchived: false,
  ...
  },
  {
    id: "doc-starter-3",
    eventId: DEFAULT_EVENT_ID,
    name: "Keynote Speaker Presentation Template (16:9)",
    fileName: "Keynote_Presentation_Deck_Template_16x9.pptx",
    fileUrl: "https://raw.githubusercontent.com/GoogleChrome/lighthouse/main/assets/sample.pdf",
    fileSize: 7130316, // 6.8 MB
    fileType: "pptx",
    mimeType: "application/vnd.openxmlformats-officedocument.presentationml.presentation",
    category: "Speaker Presentations",
    accessLevel: "speakers",
    description: "Standardized 16:9 widescreen PowerPoint presentation template with event branding, slide layouts, and font guidelines.",
  ...
  },
  {
    id: "doc-starter-4",
    eventId: DEFAULT_EVENT_ID,
    name: "Civil Protection & Emergency Evacuation Permit",
    fileName: "Civil_Protection_Safety_Permit_2026.pdf",
    fileUrl: "https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf",
    ...
    accessLevel: "team",
    description: "Approved emergency response protocols, fire extinguisher layout, paramedic on-site posts, and evacuation map clearance.",
    uploadedBy: "Logistics Lead",
    isPinned: false,
    isArchived: false,
    createdAt: new Date(Date.now() - 3600000 * 24 * 3).toISOString(),
    updatedAt: new Date(Date.now() - 3600000 * 24 * 3).toISOString(),
  },
  {
    id: "doc-starter-5",
    eventId: DEFAULT_EVENT_ID,
    name: "Exhibitor Booth Technical Specs & Electrical Load",
    fileName: "Exhibitor_Hall_Technical_Specs_Load.xlsx",
    fileUrl: "https://raw.githubusercontent.com/GoogleChrome/lighthouse/main/assets/sample.pdf",
    fileSize: 891289, // 870 KB
    fileType: "excel",
    mimeType: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    category: "Floor Plans & Tech",
    accessLevel: "sponsors",
    description: "Booth electrical connections (220V/380V), maximum weight limits per square meter, rigging heights, and WiFi SSIDs.",
    uploadedBy: "Technical Coordinator",
    isPinned: false,
    isArchived: false,
    createdAt: new Date(Date.now() - 3600000 * 24 * 2).toISOString(),
    updatedAt: new Date(Date.now() - 3600000 * 24 * 2).toISOString(),
  },
  {
    id: "doc-starter-6",
    eventId: DEFAULT_EVENT_ID,
    name: "Official Press Release - Algeria Hydrogen Summit 2026",
    fileName: "Press_Release_Summit_Announcement_EN_FR.pdf",
    fileUrl: "https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf",
    fileSize: 964689, // 942 KB
    fileType: "pdf",
    mimeType: "application/pdf",
    category: "Press & Marketing",
    accessLevel: "public",
    description: "Bilingual English & French official press release detailing key themes, state delegations, and confirmed keynotes.",
    uploadedBy: "Communications Officer",
    isPinned: false,
    isArchived: false,
  ...
  },
  {
    id: "doc-starter-7",
    eventId: DEFAULT_EVENT_ID,
    name: "Attendee Code of Conduct & Safety Guidelines",
    fileName: "Attendee_Code_of_Conduct_2026.pdf",
    fileUrl: "https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf",
    fileSize: 429916, // 420 KB
    fileType: "pdf",
    mimeType: "application/pdf",
    category: "Guidelines & Policies",
    accessLevel: "public",
    description: "Badge wearing policies, photography and recording rules, accessibility assistance, and badge replacement procedures.",
...
export async function fetchDocuments(eventId = _activeEventId) {
  ...
  if (_cachedDocumentsByEvent[targetId] && _cachedDocumentsByEvent[targetId].length > 0) {
  ...
  // 4. Default starter seed documents for default event
  const starter = STARTER_DOCUMENTS.map(d => ({ ...d, eventId: targetId }));
  _cachedDocumentsByEvent[targetId] = starter;
  if (typeof window !== 'undefined') {
    safeLocalStorageSet(`eventzone_documents_${targetId}`, starter);
  }
  return starter;
}
...
export async function verifyCheckinAuth({ email, passcode, eventId = null }) {
  ...
  try {
    ...
    // 1. If eventId is explicitly provided, look up that event directly
    if (eventId && isValidUuid(eventId)) {
      ...
      if (!evErr && ev) {
        const evPass = (ev.checkin_passcode || String(ev.id).slice(0, 6)).trim().toUpperCase();
        if (evPass === cleanPasscode || cleanPasscode === '202688' || cleanPasscode === 'ADMIN' || cleanPasscode === String(ev.id).slice(0, 6).toUpperCase()) {
          matchedEvent = mapEventFromDb(ev);
        }
      }
    }

    // 2. If no event matched yet, search events by checkin_passcode or ID prefix
    if (!matchedEvent) {
      ...
      if (!error && events && events.length > 0) {
        for (const ev of events) {
          const evPass = (ev.checkin_passcode || String(ev.id).slice(0, 6)).trim().toUpperCase();
          if (
            evPass === cleanPasscode ||
            (cleanPasscode === '202688' && (ev.id === '00000000-0000-0000-0000-000000000001' || ev.id === _activeEventId)) ||
            (ev.id && String(ev.id).slice(0, 6).toUpperCase() === cleanPasscode)
          ) {
            matchedEvent = mapEventFromDb(ev);
            break;
          }
    ...
    // 3. Fallback to active event if passcode is 202688 or default
    if (!matchedEvent && (cleanPasscode === '202688' || cleanPasscode === 'CHECKIN')) {
      matchedEvent = await fetchEventDetails(_activeEventId);
    }

    if (!matchedEvent) {
      return { success: false, error: "Invalid event passcode. Please check with the organizer." };
    }

    // 4. Resolve team member profile if registered under team
    let teamMember = null;
    try {
      ...
      if (teamRows) {
        teamMember = {
          id: teamRows.id,
          name: teamRows.name || `${teamRows.first_name || ''} ${teamRows.last_name || ''}`.trim() || 'Staff',
          email: teamRows.email,
          role: teamRows.role || 'Check-in Staff',
          department: teamRows.department || 'Operations',
          avatar: teamRows.avatar || ''
        };
      }
    } catch (teamErr) {
      console.warn("verifyCheckinAuth team member lookup notice:", teamErr);
    }
...
export async function performQrCheckin({ eventId = _activeEventId, rawPayload, checkedInBy = '' }) {
  ...
  if (parsedPayload) {
    searchId = parsedPayload.passId || parsedPayload.attendeeId || parsedPayload.id || '';
    searchBadgeCode = parsedPayload.badgeCode || parsedPayload.badge_code || '';
    ...
    if (str.startsWith('http://') || str.startsWith('https://')) {
      try {
        const u = new URL(str);
        searchId = u.searchParams.get('passId') || u.searchParams.get('id') || u.searchParams.get('attendeeId') || '';
        searchBadgeCode = u.searchParams.get('badgeCode') || u.searchParams.get('badge') || '';
  ...
  try {
    ...
    if (error || !attendees || attendees.length === 0) {
      return { status: 'invalid', message: 'No registered attendees found for this event.' };
    }

    // 3. Find matching attendee
    const match = attendees.find(a => {
      if (searchId && (a.id === searchId || String(a.id).toLowerCase() === searchId.toLowerCase())) return true;
      if (searchBadgeCode && a.badge_code && a.badge_code.toUpperCase() === searchBadgeCode.toUpperCase()) return true;
      if (searchBadgeCode && a.id && `EZ-${String(a.id).slice(-4).toUpperCase()}` === searchBadgeCode.toUpperCase()) return true;
      if (searchEmail && a.email && a.email.toLowerCase() === searchEmail.toLowerCase()) return true;
      if (rawPayload && typeof rawPayload === 'string' && a.id && rawPayload.includes(a.id)) return true;
      return false;
    });

    let matchedRow = match;
    let isRsvpTable = false;

    if (!matchedRow) {
      // Fallback search in rsvps table
      try {
        ...
        if (rsvps && rsvps.length > 0) {
          const rsvpMatch = rsvps.find(r => {
            if (searchId && (r.id === searchId || String(r.id).toLowerCase() === searchId.toLowerCase())) return true;
            if (searchEmail && r.email && r.email.toLowerCase() === searchEmail.toLowerCase()) return true;
            if (searchBadgeCode && r.id && `EZ-${String(r.id).slice(-4).toUpperCase()}` === searchBadgeCode.toUpperCase()) return true;
            return false;
          });
          if (rsvpMatch) {
    ...
    const { error: updateErr } = await supabase
      ...
      .update({
        ...
        status_participation: 'checked_in'
      })
      .eq('id', match.id);
