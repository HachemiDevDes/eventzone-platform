...
export async function createEvent(eventData, userId) {
  ...
  try {
    ...
    // Notify super admin when an organizer publishes a live event
    if (row.status === 'published') {
      try {
        if (typeof window !== "undefined") {
          ...
          }).catch(notifyErr => console.warn("Admin publish notify warning:", notifyErr));
        }
      } catch (e) {}
    }
  ...
  try {
    ...
    if (error) {
      ...
      const safePayload = {
        ...
        ...(fullPayload.portal_status ? { portal_status: fullPayload.portal_status } : {}),
      };
      if (details.slug) {
        safePayload.slug = String(details.slug).toLowerCase().trim().replace(/[^a-z0-9-]+/g, '').replace(/^-+|-+$/g, '');
      }
    ...
    if (typeof window !== "undefined") {
      try {
        const cachedObj = { ...details, id: targetId };
        safeLocalStorageSet(`eventzone_cached_event_${targetId}`, cachedObj);
        if (details?.slug) {
          safeLocalStorageSet(`eventzone_cached_event_${details.slug}`, cachedObj);
        }
      } catch (cErr) {}
    }
    ...
    // Notify super admin when event is updated to published status
    if (details?.status === 'published') {
      try {
        if (typeof window !== "undefined") {
          const prevNotified = details?.portal_settings?.admin_published_notified_at;
          if (!prevNotified) {
            fetch('/api/email/admin-notify', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                ...
                organizer: {
                  fullName: details.organizer_name || details.host_name,
                  email: details.contact_email || details.host_email,
                  companyName: details.organization,
...
function mapEventToDb(details) {
  ...
  if (details.portalSettings !== undefined || details.portal_settings !== undefined || details.is_hero_featured !== undefined || details.isHeroFeatured !== undefined || details.checkinGates !== undefined || details.checkin_gates !== undefined) {
