...
export async function fetchAttendeeConnections(userOrId, eventId) {
  ...
  try {
    const res = await fetch(`/api/events/${targetEventId}/connections?email=${encodeURIComponent(userEmail)}&userId=${encodeURIComponent(userId)}`);
    if (res.ok) {
...
export async function fetchEventChatMessages(userOrEmail, eventId) {
  ...
  try {
    const res = await fetch(`/api/events/${targetEventId}/chat?email=${encodeURIComponent(userEmail)}&userId=${encodeURIComponent(userId)}`);
    if (res.ok) {
      const data = await res.json();
