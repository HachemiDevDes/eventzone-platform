...
export async function fetchCheckinAttendees(eventId = _activeEventId) {
  ...
  try {
    ...
    if (isValidUuid(resolvedId)) {
      query = query.eq('event_id', resolvedId);
    }
    ...
    // Also fallback to rsvps table if needed
    let rsvps = [];
    try {
      let rQuery = supabase.from('rsvps').select('*');
      if (isValidUuid(resolvedId)) {
        rQuery = rQuery.eq('event_id', resolvedId);
      }
