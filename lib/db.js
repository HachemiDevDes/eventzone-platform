...
export const DEFAULT_EVENT_ID = process.env.NEXT_PUBLIC_EVENT_ID || '00000000-0000-0000-0000-000000000001';
...
// ─────────────────────────────────────────────
//  IN-MEMORY QUERY CACHE & PROMISE DEDUPLICATOR
//  Prevents duplicate queries across components and reduces egress by up to 90%
// ─────────────────────────────────────────────
const _queryMemoryCache = new Map();
const _inFlightPromises = new Map();
const DEFAULT_QUERY_TTL_MS = 25000; // 25s cache window

export function clearQueryCache(prefix = "") {
  if (!prefix) {
    _queryMemoryCache.clear();
  } else {
    for (const key of _queryMemoryCache.keys()) {
      if (key.startsWith(prefix) || key.includes(prefix)) {
        _queryMemoryCache.delete(key);
      }
    }
  }
}
...
export function invalidateQueryCache(prefix = "") {
  return clearQueryCache(prefix);
}

export async function cachedQuery(cacheKey, fnOrTtl, ttlOrFn = DEFAULT_QUERY_TTL_MS) {
  let fetchFn;
  let ttlMs = DEFAULT_QUERY_TTL_MS;
  if (typeof fnOrTtl === 'function') {
    fetchFn = fnOrTtl;
    if (typeof ttlOrFn === 'number') ttlMs = ttlOrFn;
  } else if (typeof ttlOrFn === 'function') {
    fetchFn = ttlOrFn;
    if (typeof fnOrTtl === 'number') ttlMs = fnOrTtl;
  } else {
    return null;
  }

  const now = Date.now();
  const cached = _queryMemoryCache.get(cacheKey);
  if (cached && (now - cached.timestamp) < ttlMs) {
    return cached.data;
  }

  if (_inFlightPromises.has(cacheKey)) {
    return _inFlightPromises.get(cacheKey);
  }
  ...
  const promise = (async () => {
    try {
      ...
      if (data !== undefined && data !== null) {
        _queryMemoryCache.set(cacheKey, { data, timestamp: Date.now() });
      }
      return data;
    } finally {
  ...
  _inFlightPromises.set(cacheKey, promise);
  return promise;
}
...
export async function fetchSiblingProfiles(email) {
  if (!email) return [];
  const cleanEmail = email.trim().toLowerCase();
  return cachedQuery(`sibling_profiles_${cleanEmail}`, async () => {
    try {
      const { data, error } = await supabase
        .from('profiles')
        .select('*')
      ...
      console.warn("fetchSiblingProfiles DB notice:", sibErr);
    }
    return [];
  }, 120000);
}
...
export async function upsertUserProfile(profile) {
  try {
    ...
    // Super admin role or is_admin privilege can NEVER be assigned or elevated via client profile upsert.
    // Super admin status is strictly provisioned directly in the database.
    if (profile.role && profile.role !== 'super_admin') {
      payload.role = profile.role;
    }
    delete payload.is_admin;
    if (payload.metadata && typeof payload.metadata === 'object') {
      delete payload.metadata.role;
      delete payload.metadata.is_admin;
    }
    if (payload.social_links && typeof payload.social_links === 'object') {
      delete payload.social_links.role;
      delete payload.social_links.is_admin;
    }
    ...
    if (payload.id) {
      const { data, error } = await supabase
        ...
        .single();
      if (!error && data) {
        clearQueryCache("profile_");
        clearQueryCache("sibling_profiles_");
        return data;
      }
...
export async function fetchUserEvents(userId, userEmail = null, force = false) {
  if (!userId && !userEmail) {
    return [];
  }
  const cleanEmail = (userEmail && typeof userEmail === 'string') ? userEmail.trim().toLowerCase() : '';
  const cacheKey = `user_events_${userId || ''}_${cleanEmail}`;
  if (force) {
  ...
  return cachedQuery(cacheKey, async () => {
    try {
      ...
      if (cleanEmail) {
        try {
          const siblingProfiles = await cachedQuery(`sibling_profiles_${cleanEmail}`, async () => {
      ...
      // Query team_members to find all events where this user was added/invited
      const teamEventMembership = {};
      const teamEventIds = [];
      if (cleanEmail) {
        try {
          const teamRows = await cachedQuery(`user_team_members_${cleanEmail}`, async () => {
            const { data } = await supabase
              .from('team_members')
              .select('id, event_id, role, department, permissions, status')
              .ilike('email', cleanEmail)
              .neq('status', 'archived');
            return Array.isArray(data) ? data : [];
          }, 120000);
          if (Array.isArray(teamRows) && teamRows.length > 0) {
            teamRows.forEach(r => {
              if (r.event_id && isValidUuid(r.event_id)) {
                teamEventIds.push(r.event_id);
                teamEventMembership[r.event_id] = {
                  memberId: r.id,
                  role: r.role || 'Staff',
          ...
          console.warn("fetchUserEvents team_members query note:", teamErr);
        }
      }

      let query = supabase.from('events').select('id, name, slug, tagline, category, location, type, start_date, end_date, banner, cover_url, logo_url, capacity, status, country, city, venue_name, venue_address, virtual_url, virtual_platform, schedule_mode, event_logo, organizer_logo, organizer_name, contact_email, organization, organizer_id, owner_id, created_at, updated_at');
      ...
      if (cleanEmail) {
        filterConditions.push(`contact_email.ilike.${cleanEmail}`);
      }
      if (teamEventIds.length > 0) {
        filterConditions.push(`id.in.(${teamEventIds.join(',')})`);
      }
      ...
      // Guarantee that all team events are loaded even if the or filter missed any
      if (teamEventIds.length > 0) {
        const existingIds = new Set(eventsData.map(e => e.id));
        const missingTeamIds = teamEventIds.filter(id => !existingIds.has(id));
        if (missingTeamIds.length > 0) {
          try {
            const { data: directTeamEvents } = await supabase
              .from('events')
              .select('id, name, slug, tagline, category, location, type, start_date, end_date, banner, cover_url, logo_url, capacity, status, country, city, venue_name, venue_address, virtual_url, virtual_platform, schedule_mode, event_logo, organizer_logo, organizer_name, contact_email, organization, organizer_id, owner_id, created_at, updated_at')
              .in('id', missingTeamIds);
            if (Array.isArray(directTeamEvents) && directTeamEvents.length > 0) {
              eventsData.push(...directTeamEvents);
            }
          } catch (directErr) {
            console.warn("Error fetching direct team events:", directErr);
          }
        }
      }
      ...
      if (eventsData.length === 0) {
        if (typeof window !== "undefined") {
          const cached = safeLocalStorageGet("eventzone_cache_user_events", null) || safeLocalStorageGet("eventzone_user_events", null);
      ...
      const mapped = eventsData.map(ev => {
        const base = mapEventFromDb(ev);
        if (teamEventMembership[ev.id]) {
          return {
            ...base,
            isTeamMember: true,
            teamMemberId: teamEventMembership[ev.id].memberId,
            teamRole: teamEventMembership[ev.id].role,
            teamDepartment: teamEventMembership[ev.id].department,
            teamPermissions: teamEventMembership[ev.id].permissions,
            teamStatus: teamEventMembership[ev.id].status,
          };
        }
        return base;
      });
...
export async function activateTeamMembership(userEmail, eventId = null) {
  if (!userEmail) return;
  const cleanEmail = userEmail.trim().toLowerCase();
  clearQueryCache('team_');
  clearQueryCache('user_events_');
  try {
    let q = supabase
      .from('team_members')
    ...
    if (error) console.warn("activateTeamMembership note:", error.message);
    return data;
  } catch (err) {
    console.warn("activateTeamMembership error:", err);
  }
}

export async function fetchPublicEvents() {
  return cachedQuery("public_events", async () => {
    try {
      const { data, error } = await supabase
        .from('events')
        .select('id, name, slug, tagline, category, location, type, start_date, end_date, banner, cover_url, logo_url, capacity, status, country, city, venue_name, venue_address, virtual_url, virtual_platform, schedule_mode, event_logo, organizer_logo, organizer_name, contact_email, organization, organizer_id, owner_id, created_at')
        .eq('status', 'published')
        .order('start_date', { ascending: true });
      if (error || !data) {
        if (typeof window !== "undefined") {
          const cached = safeLocalStorageGet("eventzone_cache_public_events", null) || safeLocalStorageGet("eventzone_public_events", null);
          if (Array.isArray(cached) && cached.length > 0) return cached.filter(e => e.status === 'published');
        }
        return [];
      }
      const mapped = data.map(mapEventFromDb).filter(e => e.status === 'published');
...
export const RESERVED_SLUGS = [
  "api", "checkin", "ci", "embed", "admin", "auth", "profile", "my-tickets", 
  "events-hub", "create-event", "event-landing", "overview", "calendar", 
  "event-details", "attendees", "pending", "organizations", "sponsors", 
  "exhibitors", "speakers", "opportunities", "influencers", "tickets", "forms", 
  "rsvp", "logistics", "documents", "my-team", "developers", "analytics", 
  "communications", "certificates", "floor-plan", "portal-settings",
  "en", "fr", "ar", "null", "undefined", "default-summit-2025", "myevent",
  "login", "signup", "register", "terms", "privacy", "contact", "about",
  "dashboard", "settings", "home", "cookie-settings", "cookies", 
  "enterprise-security", "security", "compliance-gdpr", "compliance", "gdpr"
];

/**
 * Check if a custom event slug is available or already taken by another event.
 * Returns availability, formatted clean slug, error message, and suggestions.
 */
export async function checkEventSlugAvailability(rawSlug, excludeEventId = null) {
  if (!rawSlug || typeof rawSlug !== "string") {
    return { available: false, cleanSlug: "", reason: "Please enter a URL for your event." };
  }
  ...
  if (!cleanSlug || cleanSlug.length < 3) {
    return { available: false, cleanSlug, reason: "Link must be at least 3 characters long." };
  }

  if (cleanSlug.length > 60) {
    return { available: false, cleanSlug, reason: "Link cannot exceed 60 characters." };
  }
  ...
  if (RESERVED_SLUGS.includes(cleanSlug)) {
    return { 
      available: false, 
      cleanSlug, 
      reason: "This link is reserved for platform use. Please choose another link.",
      suggestions: [
        `${cleanSlug}-event`,
        `${cleanSlug}-2026`,
        `${cleanSlug}-summit`
  ...
  try {
    ...
    if (excludeEventId && isValidUuid(excludeEventId)) {
    ...
    if (!error && Array.isArray(data) && data.length > 0) {
      ...
      const baseClean = cleanSlug.replace(/-\d{4}$/, "");
      const rawCandidates = [
        `${baseClean}-${year}`,
        cleanSlug.endsWith("-event") ? `${cleanSlug}-${year}` : `${cleanSlug}-event`,
        `${cleanSlug}-algiers`,
        cleanSlug.endsWith("-summit") ? `${cleanSlug}-online` : `${cleanSlug}-summit`,
        `${cleanSlug}-edition`
      ].filter((s, idx, arr) => s !== cleanSlug && arr.indexOf(s) === idx && !RESERVED_SLUGS.includes(s));

      let validSuggestions = rawCandidates.slice(0, 4);
      try {
        const { data: takenList } = await supabase
          .from('events')
          .select('slug')
          .in('slug', rawCandidates);
        if (Array.isArray(takenList) && takenList.length > 0) {
          const takenSlugs = new Set(takenList.map(t => (t.slug || "").toLowerCase()));
          validSuggestions = rawCandidates.filter(s => !takenSlugs.has(s)).slice(0, 4);
        }
      } catch {
        // Fallback to rawCandidates
      }

      return {
        available: false,
        cleanSlug,
        reason: "This link is already taken by another event. Please choose an available link.",
    ...
    console.warn("checkEventSlugAvailability error:", err);
    return { available: true, cleanSlug };
  }
}
...
export async function createEvent(eventData, userId) {
  ...
  if (!cleanSlug || cleanSlug === "myevent") {
    cleanSlug = eventData.title 
      ? String(eventData.title).toLowerCase().trim().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '')
      : `event-${Date.now()}`;
  }

  // Pre-validate slug availability to guarantee no collisions
  try {
    const slugCheck = await checkEventSlugAvailability(cleanSlug);
    if (!slugCheck.available) {
      cleanSlug = `${cleanSlug}-${Date.now().toString(36).slice(-4)}`;
    }
  } catch (slugErr) {
    console.warn("Slug pre-validation notice:", slugErr);
  }

  const resolvedOrganizerName = (eventData.organizerName || eventData.organization || eventData.hostName || '').trim();
  const resolvedHostName = (eventData.hostName || eventData.organizerName || '').trim();
  const resolvedOrganization = (eventData.organization || eventData.organizerName || '').trim();
  const resolvedContactEmail = (eventData.contactEmail || eventData.hostEmail || '').trim();
  const resolvedContactPhone = (eventData.contactPhone || '').trim();
  const resolvedWebsiteUrl = (eventData.websiteUrl || '').trim();
  const resolvedOrganizerLogo = (eventData.organizerLogo || eventData.hostLogo || '').trim();

  const initialPortalSettings = {
    networking: true,
    agenda: true,
    exhibitors: true,
    sponsors: true,
  ...
  const row = {
    ...
    portal_settings: initialPortalSettings,
    checkin_gates: [],
  ...
  try {
    const { data, error } = await supabase
      ...
      .single();
    clearQueryCache("user_events_");
    clearQueryCache("public_events");
    clearQueryCache("event_details_");
    
    const finalEvent = mapEventFromDb(data || row);
    
    if (typeof window !== "undefined") {
      try {
        safeLocalStorageSet(`eventzone_cached_event_${newId}`, finalEvent);
        if (cleanSlug) safeLocalStorageSet(`eventzone_cached_event_${cleanSlug}`, finalEvent);
      } catch (cacheErr) {}
    }
    ...
    if (row.status === 'published' && row.slug) {
      try {
        import('./indexnow.js').then(m => m.submitToIndexNow([`/${row.slug}`, '/', '/sitemap.xml'])).catch(() => {});
      } catch (e) {}
    }

    // Notify super admin when a new event is added to the platform
    try {
      if (typeof window !== "undefined") {
        fetch('/api/email/admin-notify', {
          ...
          body: JSON.stringify({
            ...
            event: {
              ...
              organizer_id: row.organizer_id || userId,
            },
            organizer: {
              id: userId,
              fullName: resolvedOrganizerName || resolvedHostName,
              email: resolvedContactEmail,
              companyName: resolvedOrganization,
              phone: resolvedContactPhone,
            }
          })
        }).catch(notifyErr => console.warn("Admin new event notify warning:", notifyErr));
      }
    } catch (e) {}
...
export async function archiveEvent(eventId) {
  try {
    ...
    const currentSettings = currentEvent?.portal_settings && typeof currentEvent.portal_settings === 'object'
...
export async function permanentDeleteEvent(eventId) {
  ...
  try {
    ...
    clearQueryCache("public_events");
    clearQueryCache("event_details_");
    clearQueryCache(`attendees_${eventId}`);
    clearQueryCache(`tickets_${eventId}`);
    clearQueryCache(`sessions_${eventId}`);
    clearQueryCache(`sponsors_${eventId}`);
    clearQueryCache(`exhibitors_${eventId}`);
    clearQueryCache(`opportunities_${eventId}`);
    clearQueryCache(`influencers_${eventId}`);
    clearQueryCache(`team_${eventId}`);
    clearQueryCache(`floorplans_${eventId}`);
    clearQueryCache(`forms_${eventId}`);
    clearQueryCache(`form_submissions_${eventId}`);
    clearQueryCache(`rsvp_settings_${eventId}`);
    clearQueryCache(`rsvps_${eventId}`);
    clearQueryCache(`logistics_${eventId}`);
...
function mapEventSummaryFromDb(row) {
  ...
  return {
    ...
    country: row.country || 'Algeria',
    city: row.city || 'Algiers',
    ...
    organizerName: row.organizer_name || row.host_name || row.organization || row.portal_settings?.organizerName || row.portal_settings?.organization || '',
    organizer_name: row.organizer_name || row.host_name || row.organization || row.portal_settings?.organizerName || '',
    hostName: row.host_name || row.organizer_name || row.portal_settings?.hostName || '',
    host_name: row.host_name || row.organizer_name || row.portal_settings?.hostName || '',
    organization: row.organization || row.organizer_name || row.portal_settings?.organization || '',
    contactEmail: row.contact_email || row.host_email || row.portal_settings?.contactEmail || '',
    contact_email: row.contact_email || row.host_email || row.portal_settings?.contactEmail || '',
    contactPhone: row.contact_phone || row.portal_settings?.contactPhone || '',
    contact_phone: row.contact_phone || row.portal_settings?.contactPhone || '',
    websiteUrl: row.website_url || row.portal_settings?.websiteUrl || '',
    website_url: row.website_url || row.portal_settings?.websiteUrl || '',
    organizerLogo: row.organizer_logo || row.host_logo || row.portal_settings?.organizerLogo || '',
    organizer_logo: row.organizer_logo || row.host_logo || row.portal_settings?.organizerLogo || '',
    hostLogo: row.host_logo || row.organizer_logo || row.portal_settings?.organizerLogo || '',
    youtubeUrl: row.youtube_url || row.video_url || '',
    videoUrl: row.video_url || row.youtube_url || '',
    gallery: parsedGallery,
    portalStatus: row.portal_status || row.portal_settings?.portal_status || row.portal_settings?.portalStatus || 'open',
    portal_status: row.portal_status || row.portal_settings?.portal_status || row.portal_settings?.portalStatus || 'open',
    portalOpenTime: row.portal_open_time || row.portal_settings?.portal_open_time || row.portal_settings?.portalOpenTime || '',
    portal_open_time: row.portal_open_time || row.portal_settings?.portal_open_time || row.portal_settings?.portalOpenTime || '',
    portalMessage: row.portal_message !== undefined && row.portal_message !== null ? row.portal_message : (row.portal_settings?.portal_message || row.portal_settings?.portalMessage || ''),
    portal_message: row.portal_message !== undefined && row.portal_message !== null ? row.portal_message : (row.portal_settings?.portal_message || row.portal_settings?.portalMessage || ''),
    portalSettings: typeof row.portal_settings === 'object' && row.portal_settings !== null ? row.portal_settings : {},
    portal_settings: typeof row.portal_settings === 'object' && row.portal_settings !== null ? row.portal_settings : {},
  };
}
...
export async function fetchEventDetails(eventId = _activeEventId) {
  ...
  return cachedQuery(`event_details_${targetId}`, async () => {
    try {
      ...
      const cleanSlugTarget = String(targetId).trim().toLowerCase().replace(/[^a-z0-9-]+/g, '');
      ...
      // 2. Targeted fallback (only if not a UUID): match by slug or title pattern
      if (!isUuid && cleanSlugTarget) {
        const { data: matchedEvents, error: matchErr } = await supabase
          .from('events')
          .select('*')
          .or(`slug.ilike.${cleanSlugTarget},name.ilike.%${cleanSlugTarget}%`)
          .limit(3);

        if (!matchErr && Array.isArray(matchedEvents) && matchedEvents.length > 0) {
          return mapEventFromDb(matchedEvents[0]);
        }
      }
...
export async function updateEventDetails(details, eventId = _activeEventId) {
  ...
  // Support both (details, eventId) and (eventId, details) cleanly
  if (typeof details === 'string') {
    targetId = details;
    targetDetails = (typeof eventId === 'object' && eventId !== null) ? eventId : {};
  } else {
    targetId = eventId || details?.id || _activeEventId;
    targetDetails = (typeof details === 'object' && details !== null) ? details : {};
  }

  const fullPayload = mapEventToDb(targetDetails);
  ...
  try {
    const { error } = await supabase
      ...
      .eq('id', targetId);
    if (error) {
      console.warn("Extended event update warning, falling back to safe columns:", error.message);
      // Safe fallback retaining all valid columns and organizer info
      const safePayload = {
        name: targetDetails.title || targetDetails.name,
        tagline: targetDetails.tagline,
        category: targetDetails.category,
        location: targetDetails.location,
        type: targetDetails.type,
        start_date: targetDetails.startDate || targetDetails.start_date,
        end_date: targetDetails.endDate || targetDetails.end_date,
        description: targetDetails.description,
        banner: targetDetails.banner,
        cover_url: targetDetails.banner || targetDetails.cover_url,
        capacity: targetDetails.capacity,
        status: targetDetails.status || 'published',
        ...(fullPayload.organizer_name ? { organizer_name: fullPayload.organizer_name } : {}),
        ...(fullPayload.host_name ? { host_name: fullPayload.host_name } : {}),
        ...(fullPayload.organization ? { organization: fullPayload.organization } : {}),
        ...(fullPayload.contact_email ? { contact_email: fullPayload.contact_email } : {}),
        ...(fullPayload.host_email ? { host_email: fullPayload.host_email } : {}),
        ...(fullPayload.contact_phone ? { contact_phone: fullPayload.contact_phone } : {}),
        ...(fullPayload.website_url ? { website_url: fullPayload.website_url } : {}),
        ...(fullPayload.organizer_logo ? { organizer_logo: fullPayload.organizer_logo } : {}),
        ...(fullPayload.host_logo ? { host_logo: fullPayload.host_logo } : {}),
        ...(fullPayload.portal_settings ? { portal_settings: fullPayload.portal_settings } : {}),
        ...(fullPayload.venue_name ? { venue_name: fullPayload.venue_name } : {}),
        ...(fullPayload.venue_address ? { venue_address: fullPayload.venue_address } : {}),
        ...(fullPayload.country ? { country: fullPayload.country } : {}),
        ...(fullPayload.city ? { city: fullPayload.city } : {}),
        ...(fullPayload.virtual_url ? { virtual_url: fullPayload.virtual_url } : {}),
        ...(fullPayload.virtual_platform ? { virtual_platform: fullPayload.virtual_platform } : {}),
        ...(fullPayload.virtual_instructions ? { virtual_instructions: fullPayload.virtual_instructions } : {}),
        ...(fullPayload.schedule_mode ? { schedule_mode: fullPayload.schedule_mode } : {}),
        ...(fullPayload.schedule_time ? { schedule_time: fullPayload.schedule_time } : {}),
        ...(fullPayload.multi_locations ? { multi_locations: fullPayload.multi_locations } : {}),
        ...(fullPayload.gallery ? { gallery: fullPayload.gallery } : {}),
        ...(fullPayload.youtube_url ? { youtube_url: fullPayload.youtube_url } : {}),
        ...(fullPayload.video_url ? { video_url: fullPayload.video_url } : {}),
        ...(fullPayload.event_logo ? { event_logo: fullPayload.event_logo } : {}),
        ...(fullPayload.logo ? { logo: fullPayload.logo } : {}),
        ...(fullPayload.logo_url ? { logo_url: fullPayload.logo_url } : {}),
        ...(fullPayload.checkin_passcode ? { checkin_passcode: fullPayload.checkin_passcode } : {}),
        ...(fullPayload.portal_status !== undefined ? { portal_status: fullPayload.portal_status } : {}),
        ...(fullPayload.portal_open_time !== undefined ? { portal_open_time: fullPayload.portal_open_time } : {}),
        ...(fullPayload.portal_message !== undefined ? { portal_message: fullPayload.portal_message } : {}),
      };
      if (targetDetails.slug) {
        safePayload.slug = String(targetDetails.slug).toLowerCase().trim().replace(/[^a-z0-9-]+/g, '').replace(/^-+|-+$/g, '');
      }
    ...
    // Invalidate memory query cache immediately
    clearQueryCache(`event_details_${targetId}`);
    clearQueryCache("user_events_");
    clearQueryCache("public_events");
    if (details?.slug) {
      clearQueryCache(`event_details_${details.slug}`);
    }

    // Persist updated details into browser cache
    if (typeof window !== "undefined") {
      try {
        const prevCached = safeLocalStorageGet(`eventzone_cached_event_${targetId}`, {});
        const cachedObj = { ...(prevCached || {}), ...targetDetails, id: targetId };
        safeLocalStorageSet(`eventzone_cached_event_${targetId}`, cachedObj);
        if (targetDetails?.slug || cachedObj.slug) {
          safeLocalStorageSet(`eventzone_cached_event_${targetDetails?.slug || cachedObj.slug}`, cachedObj);
        }
      } catch (cErr) {}
    }

    try {
      broadcastRealtimeChange('EVENT_UPDATED', { ...targetDetails, id: targetId }, targetId);
    } catch (bErr) {}

    if (details?.status === 'published' && details?.slug) {
      try {
        import('./indexnow.js').then(m => m.submitToIndexNow([`/${details.slug}`, '/', '/sitemap.xml'])).catch(() => {});
      } catch (e) {}
    }
  } catch (err) {
...
function mapEventFromDb(row) {
  ...
  let parsedGates = [];
  if (Array.isArray(row.checkin_gates)) {
    parsedGates = row.checkin_gates;
  } else if (typeof row.checkin_gates === 'string' && row.checkin_gates.trim()) {
    try {
      parsedGates = JSON.parse(row.checkin_gates);
    } catch {
      parsedGates = [];
    }
  } else if (row.portal_settings?.checkin_gates && Array.isArray(row.portal_settings.checkin_gates)) {
    parsedGates = row.portal_settings.checkin_gates;
  }
  ...
  return {
    ...
    organizerName: row.organizer_name || row.host_name || row.organization || row.portal_settings?.organizerName || row.portal_settings?.organization || row.portal_settings?.hostName || '',
    organizer_name: row.organizer_name || row.host_name || row.organization || row.portal_settings?.organizerName || '',
    hostName: row.host_name || row.organizer_name || row.portal_settings?.hostName || '',
    host_name: row.host_name || row.organizer_name || row.portal_settings?.hostName || '',
    organization: row.organization || row.organizer_name || row.portal_settings?.organization || '',
    contactEmail: row.contact_email || row.host_email || row.portal_settings?.contactEmail || row.portal_settings?.hostEmail || '',
    contact_email: row.contact_email || row.host_email || row.portal_settings?.contactEmail || '',
    contactPhone: row.contact_phone || row.portal_settings?.contactPhone || '',
    contact_phone: row.contact_phone || row.portal_settings?.contactPhone || '',
    websiteUrl: row.website_url || row.portal_settings?.websiteUrl || '',
    ...
    organizerLogo: row.organizer_logo || row.host_logo || row.portal_settings?.organizerLogo || row.portal_settings?.hostLogo || '',
    organizer_logo: row.organizer_logo || row.host_logo || row.portal_settings?.organizerLogo || '',
    hostLogo: row.host_logo || row.organizer_logo || row.portal_settings?.organizerLogo || '',
    host_logo: row.host_logo || row.organizer_logo || row.portal_settings?.organizerLogo || '',
    ...
    checkinGates: parsedGates,
    checkin_gates: parsedGates,
    gallery: parsedGallery,
    portalStatus: row.portal_status || row.portal_settings?.portal_status || row.portal_settings?.portalStatus || 'open',
    portal_status: row.portal_status || row.portal_settings?.portal_status || row.portal_settings?.portalStatus || 'open',
    portalOpenTime: row.portal_open_time || row.portal_settings?.portal_open_time || row.portal_settings?.portalOpenTime || '',
    ...
    ownerId: row.owner_id || row.organizer_id || null,
    owner_id: row.owner_id || row.organizer_id || null,
    is_hero_featured: Boolean(row.portal_settings?.is_hero_featured ?? row.is_hero_featured ?? false),
    isHeroFeatured: Boolean(row.portal_settings?.is_hero_featured ?? row.is_hero_featured ?? false),
...
function mapEventToDb(details) {
  ...
  if (details.portalSettings !== undefined || details.portal_settings !== undefined || details.portalStatus !== undefined || details.portal_status !== undefined || details.portalOpenTime !== undefined || details.portal_open_time !== undefined || details.portalMessage !== undefined || details.portal_message !== undefined || details.is_hero_featured !== undefined || details.isHeroFeatured !== undefined || details.checkinGates !== undefined || details.checkin_gates !== undefined) {
    ...
    const rawGates = details.checkinGates !== undefined ? details.checkinGates : (details.checkin_gates !== undefined ? details.checkin_gates : existingSettings.checkin_gates);
    const resolvedPortalStatus = payload.portal_status || details.portalStatus || details.portal_status || existingSettings.portal_status || existingSettings.portalStatus || 'open';
    const resolvedPortalOpenTime = payload.portal_open_time !== undefined ? payload.portal_open_time : (details.portalOpenTime || details.portal_open_time || existingSettings.portal_open_time || existingSettings.portalOpenTime || null);
    const resolvedPortalMessage = payload.portal_message !== undefined ? payload.portal_message : (details.portalMessage !== undefined ? details.portalMessage : (existingSettings.portal_message !== undefined ? existingSettings.portal_message : ''));

    payload.portal_settings = {
      ...existingSettings,
      portal_status: resolvedPortalStatus,
      portalStatus: resolvedPortalStatus,
      portal_open_time: resolvedPortalOpenTime,
      portalOpenTime: resolvedPortalOpenTime,
      portal_message: resolvedPortalMessage,
      portalMessage: resolvedPortalMessage,
      ...
      checkin_gates: Array.isArray(rawGates) ? rawGates : []
    };
  }
  if (details.checkinGates !== undefined || details.checkin_gates !== undefined) {
    const rawGates = details.checkinGates !== undefined ? details.checkinGates : details.checkin_gates;
    payload.checkin_gates = Array.isArray(rawGates) ? rawGates : [];
  }
  ...
  const resolvedOrgName = details.organizerName !== undefined ? details.organizerName : (details.organizer_name !== undefined ? details.organizer_name : (details.organization || details.hostName || details.host_name || undefined));
  if (resolvedOrgName !== undefined) {
    payload.organizer_name = resolvedOrgName;
    payload.host_name = details.hostName || details.host_name || resolvedOrgName;
    payload.organization = details.organization || resolvedOrgName;
  }
  const resolvedEmail = details.contactEmail !== undefined ? details.contactEmail : (details.contact_email !== undefined ? details.contact_email : (details.hostEmail || details.host_email || undefined));
  if (resolvedEmail !== undefined) {
    payload.contact_email = resolvedEmail;
    payload.host_email = resolvedEmail;
  }
  const resolvedPhone = details.contactPhone !== undefined ? details.contactPhone : (details.contact_phone !== undefined ? details.contact_phone : undefined);
  if (resolvedPhone !== undefined) {
    payload.contact_phone = resolvedPhone;
  }
  const resolvedWebsite = details.websiteUrl !== undefined ? details.websiteUrl : (details.website_url !== undefined ? details.website_url : undefined);
  ...
  const resolvedLogo = details.organizerLogo !== undefined ? details.organizerLogo : (details.organizer_logo !== undefined ? details.organizer_logo : (details.hostLogo || details.host_logo || undefined));
  if (resolvedLogo !== undefined) {
    payload.organizer_logo = resolvedLogo;
    payload.host_logo = resolvedLogo;
  }

  // Also ensure portal_settings embeds organizer info for complete persistence
  const currentSettings = payload.portal_settings || details.portalSettings || details.portal_settings || {};
  payload.portal_settings = {
    ...currentSettings,
    ...(payload.organizer_name !== undefined ? { organizerName: payload.organizer_name, organization: payload.organization || payload.organizer_name, hostName: payload.host_name || payload.organizer_name } : {}),
    ...(payload.contact_email !== undefined ? { contactEmail: payload.contact_email, hostEmail: payload.contact_email } : {}),
    ...(payload.contact_phone !== undefined ? { contactPhone: payload.contact_phone } : {}),
    ...(payload.website_url !== undefined ? { websiteUrl: payload.website_url } : {}),
    ...(payload.organizer_logo !== undefined ? { organizerLogo: payload.organizer_logo, hostLogo: payload.organizer_logo } : {}),
  };
  if (details.gallery !== undefined) payload.gallery = currentGallery;
  if (details.organizer_id !== undefined || details.organizerId !== undefined) {
    payload.organizer_id = details.organizer_id || details.organizerId || null;
  }
...
export async function fetchSessions(eventId = _activeEventId) {
  const targetId = eventId || _activeEventId;
  if (!targetId) return [];
  return cachedQuery(`sessions_${targetId}`, async () => {
    let data = null;
    let error = null;
    try {
    ...
    if (error || !data) {
      if (error) console.warn("fetchSessions error:", error.message || error);
      if (typeof window !== "undefined") {
        try {
          const cached = localStorage.getItem(`eventzone_cache_sessions_${targetId}`);
          if (cached) {
            const parsed = JSON.parse(cached);
...
export async function fetchAttendees(eventId = _activeEventId) {
  const targetId = eventId || _activeEventId;
  if (!targetId) return [];
  return cachedQuery(`attendees_${targetId}`, async () => {
    let query = supabase
      .from('participants')
      .select('*')
    ...
    try {
      if (isValidUuid(targetId)) {
        subsData = await cachedQuery(`attendee_subs_${targetId}`, async () => {
    ...
    if (error || !data) {
      if (error) console.warn("fetchAttendees error:", error.message || error);
      if (typeof window !== "undefined") {
        try {
          const cached = localStorage.getItem(`eventzone_cache_attendees_${targetId}`);
          if (cached) {
            const parsed = JSON.parse(cached);
            if (Array.isArray(parsed) && parsed.length > 0) return parsed;
          }
...
export async function upsertAttendee(attendee, eventId = _activeEventId) {
  let targetId = eventId || _activeEventId;
  clearQueryCache(`attendees_${targetId}`);
  clearQueryCache(`attendee_subs_${targetId}`);
  clearQueryCache("visitor_regs_");
  let row = mapAttendeeToDb(attendee, targetId);
  try {
    ...
    // If foreign key constraint violation on event_id, retry with DEFAULT_EVENT_ID
    if (error && (error.code === '23503' || String(error.message).includes('foreign key'))) {
      console.warn("Foreign key error on event_id in upsertAttendee, falling back to DEFAULT_EVENT_ID:", error.message);
      targetId = DEFAULT_EVENT_ID;
      row = mapAttendeeToDb(attendee, DEFAULT_EVENT_ID);
    ...
    if (error) {
      ...
      return { ...attendee, id: row.id, _syncFailed: true, _error: error.message };
    }
    ...
    return { ...attendee, id: row.id, _syncFailed: true, _error: e.message };
  }
}

export async function bulkUpsertAttendees(attendeesList = [], eventId = _activeEventId) {
  const targetId = eventId || _activeEventId;
  clearQueryCache(`attendees_${targetId}`);
  clearQueryCache(`attendee_subs_${targetId}`);
  clearQueryCache("visitor_regs_");
  if (!Array.isArray(attendeesList) || attendeesList.length === 0) return [];

  const results = [];
  for (const attendee of attendeesList) {
    try {
      const saved = await upsertAttendee(attendee, targetId);
      results.push(saved);
    } catch (err) {
      console.warn("bulkUpsertAttendees fallback for:", attendee.email, err);
      results.push({
        ...attendee,
        id: attendee.id || generateUuid(),
        badgeCode: attendee.badgeCode || `EZ-${Math.random().toString(36).substring(2, 6).toUpperCase()}`,
        status: attendee.status || "registered",
        registeredDate: attendee.registeredDate || new Date().toISOString(),
      });
    }
  }
  return results;
...
export async function archiveAttendee(idOrItem, emailParam, eventId = _activeEventId) {
  ...
  try {
    ...
    broadcastRealtimeChange('ATTENDEE_SAVED', { id, email, status: 'archived', status_participation: 'archived', isArchived: true }, targetId);
  } catch (e) {
    console.warn("archiveAttendee error:", e);
  }
}

export async function restoreAttendee(idOrItem, emailParam, eventId = _activeEventId) {
  const targetId = eventId || _activeEventId;
  clearQueryCache(`attendees_${targetId}`);
  clearQueryCache(`attendee_subs_${targetId}`);
  clearQueryCache("visitor_regs_");
  ...
  try {
    if (id && isValidUuid(id)) {
      await supabase.from('participants').update({ status_participation: 'registered' }).eq('id', id);
    }
    if (email && email.includes('@')) {
      const cleanEmail = email.trim().toLowerCase();
      let query = supabase.from('participants').update({ status_participation: 'registered' }).ilike('email', cleanEmail);
      if (isValidUuid(targetId)) query = query.eq('event_id', targetId);
      await query;
    }
    broadcastRealtimeChange('ATTENDEE_SAVED', { id, email, status: 'registered', status_participation: 'registered', isArchived: false }, targetId);
  } catch (e) {
    console.warn("restoreAttendee error:", e);
  }
}
...
function mapAttendeeFromDb(row) {
  ...
  return {
    ...
    isArchived: row.status_participation === 'archived' || row.status === 'archived',
    ...
    checkinGate: row.checkin_gate || (row.checked_in_by && row.checked_in_by.includes('(') ? row.checked_in_by.match(/\((.*?)\)/)?.[1] : '') || (isCheckedIn ? (row.checked_in_by === 'Badge Print' ? 'Badge Print' : 'Principal Gate') : ''),
    checkin_gate: row.checkin_gate || (row.checked_in_by && row.checked_in_by.includes('(') ? row.checked_in_by.match(/\((.*?)\)/)?.[1] : '') || (isCheckedIn ? (row.checked_in_by === 'Badge Print' ? 'Badge Print' : 'Principal Gate') : ''),
...
function mapAttendeeToDb(attendee, eventId = _activeEventId) {
  ...
  let safeRegisteredAt = new Date().toISOString();
  const rawDate = attendee.registeredDate || attendee.registered_at;
  if (rawDate) {
    const parsedDate = new Date(rawDate);
    if (!isNaN(parsedDate.getTime())) {
      safeRegisteredAt = parsedDate.toISOString();
    }
  }

  const resolvedEventId = isValidUuid(eventId)
    ? eventId
    : (isValidUuid(attendee.eventId || attendee.event_id)
        ? (attendee.eventId || attendee.event_id)
        : (isValidUuid(DEFAULT_EVENT_ID) ? DEFAULT_EVENT_ID : undefined));

  const isArchived = Boolean(attendee.isArchived || attendee.status === 'archived' || attendee.status_participation === 'archived');
  ...
  const payload = {
    ...
    status_participation: isArchived ? 'archived' : (isCheckedIn ? 'checked_in' : (attendee.status || 'registered')),
    registered_at: safeRegisteredAt,
  ...
  if (attendee.checkinGate !== undefined || attendee.checkin_gate !== undefined) {
    payload.checkin_gate = attendee.checkinGate || attendee.checkin_gate || null;
  }
...
export async function fetchPending(eventId = _activeEventId) {
  const targetId = eventId || _activeEventId;
  if (!targetId) return [];
  return cachedQuery(`pending_${targetId}`, async () => {
    let query = supabase
    ...
    if (error || !data) {
      if (error) console.warn("fetchPending error:", error.message || error);
      if (typeof window !== "undefined") {
        try {
          const cached = localStorage.getItem(`eventzone_cache_pending_${targetId}`);
          if (cached) {
            const parsed = JSON.parse(cached);
            if (Array.isArray(parsed) && parsed.length > 0) return parsed;
          }
    ...
    const mapped = (data || []).map(mapPendingFromDb);
    if (mapped.length === 0 && typeof window !== "undefined") {
      try {
        const cached = localStorage.getItem(`eventzone_cache_pending_${targetId}`);
        if (cached) {
...
export async function upsertPending(item, eventId = _activeEventId) {
  const targetId = eventId || _activeEventId;
  clearQueryCache(`pending_${targetId}`);
...
export async function fetchOrganizations(eventId = _activeEventId) {
  const targetId = eventId || _activeEventId;
  if (!targetId) return [];
  return cachedQuery(`organizations_${targetId}`, async () => {
    let data = null;
    let error = null;
    try {
      let query = supabase
    ...
    if (error || !data) {
      if (error) console.warn("fetchOrganizations error:", error.message || error);
      if (typeof window !== "undefined") {
        try {
...
function mapOrgFromDb(row) {
  return {
    ...
    nif: row.nif || row.fiscal_details?.nif || '',
    rc: row.rc || row.fiscal_details?.rc || '',
    nis: row.nis || row.fiscal_details?.nis || '',
    articleImposition: row.article_imposition || row.fiscal_details?.article_imposition || '',
    article_imposition: row.article_imposition || row.fiscal_details?.article_imposition || '',
    legalName: row.legal_name || row.fiscal_details?.legal_name || row.name || '',
    legal_name: row.legal_name || row.fiscal_details?.legal_name || row.name || '',
    legalAddress: row.legal_address || row.fiscal_details?.legal_address || row.address || '',
    legal_address: row.legal_address || row.fiscal_details?.legal_address || row.address || '',
    invoicingEmail: row.invoicing_email || row.fiscal_details?.invoicing_email || row.email || '',
    invoicing_email: row.invoicing_email || row.fiscal_details?.invoicing_email || row.email || '',
    invoicingPhone: row.invoicing_phone || row.fiscal_details?.invoicing_phone || row.phone || '',
    invoicing_phone: row.invoicing_phone || row.fiscal_details?.invoicing_phone || row.phone || '',
    bankName: row.bank_name || row.fiscal_details?.bank_name || '',
    bank_name: row.bank_name || row.fiscal_details?.bank_name || '',
    rib: row.rib || row.fiscal_details?.rib || '',
    fiscalDetails: row.fiscal_details || {},
  };
}
...
function mapOrgToDb(org, eventId = _activeEventId) {
  ...
  const nif = (org.nif || org.fiscalDetails?.nif || '').trim();
  const rc = (org.rc || org.fiscalDetails?.rc || '').trim();
  const nis = (org.nis || org.fiscalDetails?.nis || '').trim();
  const article_imposition = (org.articleImposition || org.article_imposition || org.fiscalDetails?.article_imposition || '').trim();
  const legal_name = (org.legalName || org.legal_name || org.fiscalDetails?.legal_name || org.name || '').trim();
  const legal_address = (org.legalAddress || org.legal_address || org.fiscalDetails?.legal_address || org.address || '').trim();
  const invoicing_email = (org.invoicingEmail || org.invoicing_email || org.fiscalDetails?.invoicing_email || '').trim();
  const invoicing_phone = (org.invoicingPhone || org.invoicing_phone || org.fiscalDetails?.invoicing_phone || '').trim();
  const bank_name = (org.bankName || org.bank_name || org.fiscalDetails?.bank_name || '').trim();
  const rib = (org.rib || org.fiscalDetails?.rib || '').trim();
...
export async function fetchSponsors(eventId = _activeEventId) {
  const targetId = eventId || _activeEventId;
  if (!targetId) return [];
  return cachedQuery(`sponsors_${targetId}`, async () => {
    let data = null;
    let error = null;
    try {
      const res = await supabase
    ...
    if (error || !data) {
      if (error) console.warn("fetchSponsors error:", error.message || error);
      if (typeof window !== "undefined") {
        try {
          const cached = localStorage.getItem(`eventzone_cache_sponsors_${targetId}`);
          if (cached) {
            const parsed = JSON.parse(cached);
            if (Array.isArray(parsed) && parsed.length > 0) return parsed;
          }
...
export async function fetchExhibitors(eventId = _activeEventId) {
  const targetId = eventId || _activeEventId;
  if (!targetId) return [];
  return cachedQuery(`exhibitors_${targetId}`, async () => {
    let data = null;
    let error = null;
    try {
      const res = await supabase
    ...
    if (error || !data) {
      if (error) console.warn("fetchExhibitors error:", error.message || error);
      if (typeof window !== "undefined") {
        try {
          const cached = localStorage.getItem(`eventzone_cache_exhibitors_${targetId}`);
          if (cached) {
            const parsed = JSON.parse(cached);
            if (Array.isArray(parsed) && parsed.length > 0) return parsed;
          }
...
export async function upsertExhibitor(exhibitor, eventId = _activeEventId) {
  ...
  try {
    ...
    if (error) {
      ...
      // If error is due to missing specific_equipment column on remote Postgres, retry without it
      if (error.message && (error.message.includes('specific_equipment') || error.message.includes('column'))) {
        const fallbackRow = { ...row };
        delete fallbackRow.specific_equipment;
        const retry = await supabase.from('exhibitors').upsert(fallbackRow, { onConflict: 'id' }).select().single();
        if (!retry.error && retry.data) {
          return { ...mapExhibitorFromDb(retry.data), ...exhibitor };
        }
      }
...
function mapExhibitorFromDb(row) {
  return {
    ...
    notes: row.notes || '',
    orgId: row.org_id || null,
    specificEquipment: Array.isArray(row.specific_equipment) ? row.specific_equipment : (Array.isArray(row.specificEquipment) ? row.specificEquipment : []),
    isArchived: row.status === 'archived',
    status: row.status || 'active',
  };
}
...
function mapExhibitorToDb(exhibitor, eventId = _activeEventId) {
  ...
  return {
    ...
    specific_equipment: Array.isArray(exhibitor.specificEquipment) ? exhibitor.specificEquipment : (Array.isArray(exhibitor.specific_equipment) ? exhibitor.specific_equipment : []),
...
export async function fetchOpportunities(eventId = _activeEventId) {
  const targetId = eventId || _activeEventId;
  if (!targetId) return [];
  return cachedQuery(`opportunities_${targetId}`, async () => {
    let data = null;
    let error = null;
    try {
      let query = supabase
    ...
    if (error || !data) {
      if (error) console.warn("fetchOpportunities error:", error.message || error);
      if (typeof window !== "undefined") {
        try {
          const cached = localStorage.getItem(`eventzone_cache_opportunities_${targetId}`);
          if (cached) {
            const parsed = JSON.parse(cached);
            if (Array.isArray(parsed) && parsed.length > 0) return parsed;
          }
    ...
    const mapped = data.map(mapOpportunityFromDb);
    if (mapped.length === 0 && typeof window !== "undefined") {
      try {
        const cached = localStorage.getItem(`eventzone_cache_opportunities_${targetId}`);
        if (cached) {
...
export async function upsertOpportunity(opp, eventId = _activeEventId) {
  const targetId = eventId || _activeEventId;
  clearQueryCache(`opportunities_${targetId}`);
...
function mapOpportunityFromDb(row) {
  ...
  return {
    ...
    articleImposition: row.article_imposition || row.fiscal_details?.article_imposition || '',
    article_imposition: row.article_imposition || row.fiscal_details?.article_imposition || '',
    legalName: row.legal_name || row.fiscal_details?.legal_name || row.company_name || row.name || '',
    legal_name: row.legal_name || row.fiscal_details?.legal_name || row.company_name || row.name || '',
    legalAddress: row.legal_address || row.fiscal_details?.legal_address || '',
    legal_address: row.legal_address || row.fiscal_details?.legal_address || '',
    invoicingEmail: row.invoicing_email || row.fiscal_details?.invoicing_email || row.contact_email || '',
    invoicing_email: row.invoicing_email || row.fiscal_details?.invoicing_email || row.contact_email || '',
    invoicingPhone: row.invoicing_phone || row.fiscal_details?.invoicing_phone || row.contact_phone || '',
    invoicing_phone: row.invoicing_phone || row.fiscal_details?.invoicing_phone || row.contact_phone || '',
    bankName: row.bank_name || row.fiscal_details?.bank_name || '',
    bank_name: row.bank_name || row.fiscal_details?.bank_name || '',
    rib: row.rib || row.fiscal_details?.rib || '',
    fiscalDetails: row.fiscal_details || {},
...
function mapOpportunityToDb(opp, eventId = _activeEventId) {
  ...
  const nif = (opp.nif || opp.fiscalDetails?.nif || '').trim();
  const rc = (opp.rc || opp.fiscalDetails?.rc || '').trim();
  const nis = (opp.nis || opp.fiscalDetails?.nis || '').trim();
  const article_imposition = (opp.articleImposition || opp.article_imposition || opp.fiscalDetails?.article_imposition || '').trim();
  const legal_name = (opp.legalName || opp.legal_name || opp.fiscalDetails?.legal_name || opp.companyName || opp.name || '').trim();
  const legal_address = (opp.legalAddress || opp.legal_address || opp.fiscalDetails?.legal_address || '').trim();
  const invoicing_email = (opp.invoicingEmail || opp.invoicing_email || opp.fiscalDetails?.invoicing_email || opp.contactEmail || '').trim();
  const invoicing_phone = (opp.invoicingPhone || opp.invoicing_phone || opp.fiscalDetails?.invoicing_phone || opp.contactPhone || '').trim();
  const bank_name = (opp.bankName || opp.bank_name || opp.fiscalDetails?.bank_name || '').trim();
  const rib = (opp.rib || opp.fiscalDetails?.rib || '').trim();
...
export async function fetchInfluencers(eventId = _activeEventId) {
  const targetId = eventId || _activeEventId;
  if (!targetId) return [];
  return cachedQuery(`influencers_${targetId}`, async () => {
    let data = null;
    let error = null;
    try {
      let query = supabase
    ...
    if (error || !data) {
      if (error) console.warn("fetchInfluencers error:", error.message || error);
      if (typeof window !== "undefined") {
        try {
          const cached = localStorage.getItem(`eventzone_cache_influencers_${targetId}`);
          if (cached) {
            const parsed = JSON.parse(cached);
            if (Array.isArray(parsed) && parsed.length > 0) return parsed;
          }
    ...
    const mapped = (data || []).map(mapInfluencerFromDb);
    if (mapped.length === 0 && typeof window !== "undefined") {
      try {
        const cached = localStorage.getItem(`eventzone_cache_influencers_${targetId}`);
        if (cached) {
...
export async function upsertInfluencer(influencer, eventId = _activeEventId) {
  const targetId = eventId || _activeEventId;
  clearQueryCache(`influencers_${targetId}`);
...
export async function fetchTickets(eventId = _activeEventId) {
  const targetId = eventId || _activeEventId;
  if (!targetId) return [];
  return cachedQuery(`tickets_${targetId}`, async () => {
    let data = null;
    let error = null;
    try {
      const res = await supabase
    ...
    if (error || !data) {
      if (error) console.warn("fetchTickets error:", error.message || error);
      if (typeof window !== "undefined") {
        try {
          const cached = localStorage.getItem(`eventzone_cache_tickets_${targetId}`);
          if (cached) {
            const parsed = JSON.parse(cached);
            if (Array.isArray(parsed) && parsed.length > 0) return parsed;
          }
...
export async function fetchTeam(eventId = _activeEventId) {
  const targetId = eventId || _activeEventId;
  if (!targetId) return [];
  return cachedQuery(`team_${targetId}`, async () => {
    const { data, error } = await supabase
      .from('team_members')
      .select('*')
      .eq('event_id', targetId)
...
export async function upsertTeamMember(member, eventId = _activeEventId) {
  const targetId = eventId || _activeEventId;
  clearQueryCache(`team_${targetId}`);
...
function mapTeamToDb(member, eventId = _activeEventId) {
  ...
  return {
    id: validId,
    event_id: isValidUuid(eventId) ? eventId : undefined,
    first_name: member.firstName || (member.name ? member.name.split(' ')[0] : '') || 'Team',
    last_name: member.lastName || (member.name ? member.name.split(' ').slice(1).join(' ') : '') || 'Member',
...
export async function fetchFloorPlans(eventId = _activeEventId) {
  const targetId = eventId || _activeEventId;
  if (!targetId) return [];
  return cachedQuery(`floorplans_${targetId}`, async () => {
    const { data, error } = await supabase
      .from('floor_plans')
      .select('*')
      .eq('event_id', targetId)
...
export async function fetchSingleFloorPlan(planId) {
  if (!planId) return null;
  return cachedQuery(`floorplan_single_${planId}`, async () => {
    try {
      const { data, error } = await supabase
        .from('floor_plans')
        .select('*')
        ...
        .maybeSingle();
      if (error) {
        console.warn("fetchSingleFloorPlan error:", error.message);
        return null;
      }
      return data ? mapFloorPlanFromDb(data) : null;
    } catch (err) {
      console.warn("fetchSingleFloorPlan exception:", err);
      return null;
    }
  }, 30000);
}
...
export async function upsertFloorPlan(plan, eventId = _activeEventId) {
  const targetId = eventId || _activeEventId;
  clearQueryCache(`floorplans_${targetId}`);
...
export async function archiveFloorPlan(id, eventId = _activeEventId) {
  if (!id || !isValidUuid(id)) return;
  const targetId = eventId || _activeEventId;
  clearQueryCache(`floorplans_${targetId}`);
  try {
...
export async function restoreFloorPlan(id, eventId = _activeEventId) {
  if (!id || !isValidUuid(id)) return;
  const targetId = eventId || _activeEventId;
  clearQueryCache(`floorplans_${targetId}`);
  try {
...
export async function permanentDeleteFloorPlan(id, eventId = _activeEventId) {
  if (!id || !isValidUuid(id)) return;
  const targetId = eventId || _activeEventId;
  clearQueryCache(`floorplans_${targetId}`);
  try {
...
export async function deleteFloorPlan(id, eventId = _activeEventId) {
  return await permanentDeleteFloorPlan(id, eventId);
}
...
export async function fetchVisitorRegistrations(userEmail) {
  ...
  return cachedQuery(`visitor_regs_${cleanEmail}`, async () => {
    try {
      ...
      if (eventIds.length > 0) {
        const sortedIds = [...eventIds].sort().join('_');
        const eventsList = await cachedQuery(`events_summary_${sortedIds}`, async () => {
          try {
            const { data, error } = await supabase
              .from('events')
              .select('*')
              .in('id', eventIds);
            if (error) {
              console.warn("fetchVisitorRegistrations events fetch error:", error);
            }
            return data || [];
          } catch (e) {
            console.warn("fetchVisitorRegistrations events query exception:", e);
            return [];
          }
        }, 60000);
        ...
        if (Array.isArray(eventsList)) {
          eventsList.forEach(ev => {
            ...
            if (ev.slug) eventsMap[ev.slug] = ev;
          });
        }
      }

      // Check localStorage cached events as fallback if any event details are missing
      if (typeof window !== "undefined") {
        try {
          const cachedPublic = safeLocalStorageGet("eventzone_cache_public_events", null) || safeLocalStorageGet("eventzone_public_events", null) || [];
          if (Array.isArray(cachedPublic)) {
            cachedPublic.forEach(ev => {
              if (ev.id && !eventsMap[ev.id]) eventsMap[ev.id] = ev;
              if (ev.slug && !eventsMap[ev.slug]) eventsMap[ev.slug] = ev;
            });
          }
        } catch {}
      }
      ...
      return participants.map(row => {
        const ev = eventsMap[row.event_id] || eventsMap[String(row.event_id).toLowerCase()] || {};
        const fullName = [row.first_name, row.last_name].filter(Boolean).join(' ') || row.name || '';
        const userImg = row.image || row.avatar || row.avatar_url || '';
        const resolvedTitle = ev.name || ev.title || row.event_title || row.event_name || row.eventName || "Event Registration";
        const resolvedLocation = ev.location || ev.venue_name || ev.venue_address || "Venue TBA";
        const resolvedStartDate = ev.start_date || ev.startDate || "";
        const resolvedEndDate = ev.end_date || ev.endDate || "";
        const resolvedBadgeUrl = row.template_url || row.badge_url || ev.badge_url || ev.portal_settings?.badge_url || ev.portal_settings?.badgeUrl || "";
        const resolvedBadgeSettings = row.badge_settings || ev.badge_settings || ev.portal_settings?.badge_settings || ev.portal_settings?.badgeSettings || {};
        ...
        return {
          ...
          badgeCode: row.badge_code || `PASS-${(row.id || '').toString().slice(-6).toUpperCase()}`,
          templateUrl: resolvedBadgeUrl,
...
export async function registerVisitorForEvent(eventId, visitorData) {
  ...
  if (isPending) {
    ...
    return {
      ...
      attendeeName: visitorData.name || 'Attendee',
      name: visitorData.name || 'Attendee',
      email: visitorData.email || 'visitor@eventzone.io',
      ticketType: visitorData.ticketType || visitorData.ticket_type || 'Standard Admission',
      ticketId: visitorData.ticketId || null,
      status: 'pending',
...
export async function fetchForms(eventId = _activeEventId) {
  const targetId = eventId || _activeEventId;
  if (!targetId) return [];
  return cachedQuery(`forms_${targetId}`, async () => {
    if (isValidUuid(targetId)) {
      try {
        const { data, error } = await supabase
          .from('forms')
...
export async function upsertForm(form, eventId = _activeEventId) {
  const targetId = eventId || _activeEventId;
  clearQueryCache(`forms_${targetId}`);
...
export async function fetchFormSubmissions(eventId = _activeEventId, formId = null) {
  const targetId = eventId || _activeEventId;
  if (!targetId) return [];
  const cacheKey = `form_submissions_${targetId}_${formId || 'all'}`;
...
export async function submitFormResponse(submission, eventId = _activeEventId) {
  const targetId = eventId || _activeEventId;
  clearQueryCache(`form_submissions_${targetId}`);
  clearQueryCache(`attendees_${targetId}`);
  clearQueryCache(`attendee_subs_${targetId}`);
...
export async function fetchRSVPs(eventId = _activeEventId) {
  const targetId = eventId || _activeEventId;
  return cachedQuery(`rsvps_${targetId}`, 25000, async () => {
    if (isValidUuid(targetId)) {
      try {
        const { data, error } = await supabase
          .from('rsvps')
...
export async function fetchRSVPSettings(eventId = _activeEventId) {
  ...
  return cachedQuery(`rsvp_settings_${targetId}`, 30000, async () => {
  ...
  if (isValidUuid(targetId)) {
    try {
      ...
      if (!error && data) {
        ...
        invalidateQueryCache(`rsvp_settings_${targetId}`);
  ...
  if (isValidUuid(targetId)) {
    try {
      ...
      if (!error && data) {
        ...
        invalidateQueryCache(`rsvps_${targetId}`);
...
export async function fetchLogistics(eventId = _activeEventId) {
  ...
  return cachedQuery(`logistics_${targetId}`, 30000, async () => {
...
export async function upsertFullLogistics(logisticsData, eventId = _activeEventId) {
  ...
  invalidateQueryCache(`logistics_${targetId}`);
  if (typeof window !== 'undefined') {
...
export async function fetchDocuments(eventId = _activeEventId) {
  ...
  return cachedQuery(`documents_${targetId}`, 30000, async () => {
...
export async function upsertDocument(doc, eventId = _activeEventId) {
  ...
  invalidateQueryCache(`documents_${targetId}`);
  if (typeof window !== 'undefined') {
...
export async function verifyCheckinAuth({ email, passcode, eventId = null }) {
  ...
  try {
    ...
    // 2. If no eventId, look up events whose passcode matches cleanPasscode (primary or in checkin_gates)
    if (candidateEvents.length === 0) {
      ...
      // Check checkin_gates JSONB column
      if (candidateEvents.length === 0) {
        try {
          const { data: matchedByGates } = await supabase
            .from('events')
            .select('*')
            .filter('checkin_gates', 'cs', JSON.stringify([{ passcode: cleanPasscode }]));
          if (matchedByGates && matchedByGates.length > 0) {
            candidateEvents = matchedByGates;
          }
        } catch {}
      }

      // Fallback: search recent events in case checkin_gates is stored in portal_settings or not GIN indexed
      if (candidateEvents.length === 0) {
        try {
          ...
          if (recentEvents && recentEvents.length > 0) {
            const foundInRecent = recentEvents.filter(ev => {
              const gates = Array.isArray(ev.checkin_gates) 
                ? ev.checkin_gates 
                : (ev.portal_settings?.checkin_gates && Array.isArray(ev.portal_settings.checkin_gates) ? ev.portal_settings.checkin_gates : []);
              return gates.some(g => (g.passcode || '').trim().toUpperCase() === cleanPasscode);
            });
            if (foundInRecent.length > 0) {
              candidateEvents = foundInRecent;
            }
          }
        } catch {}
      }
    ...
    // 3. For candidate events, verify staff/organizer authorization and resolve gate
    ...
    for (const rawEv of candidateEvents) {
      ...
      const isPrincipal = (evPass && evPass === cleanPasscode) || (syntheticPass && syntheticPass === cleanPasscode);

      const rawGates = Array.isArray(rawEv.checkin_gates)
        ? rawEv.checkin_gates
        : (rawEv.portal_settings?.checkin_gates && Array.isArray(rawEv.portal_settings.checkin_gates) ? rawEv.portal_settings.checkin_gates : []);
      
      const customGateMatch = rawGates.find(g => (g.passcode || '').trim().toUpperCase() === cleanPasscode);
      ...
      if (isPrincipal) {
        currentGate = {
          id: "principal",
          name: "Principal Gate",
          passcode: evPass || syntheticPass,
          isPrincipal: true,
        };
      } else if (customGateMatch) {
        currentGate = {
          id: customGateMatch.id || `gate_${customGateMatch.passcode}`,
          name: customGateMatch.name || "Custom Gate",
          passcode: customGateMatch.passcode,
          isPrincipal: false,
        };
      }

      if (!currentGate && eventId) {
        continue;
      }
      ...
      // If currentGate is valid, allow as Gate Staff terminal
      if (currentGate) {
        matchedEvent = mapEventFromDb(rawEv);
        matchedGate = currentGate;
        matchedStaff = {
    ...
    // Fallback: If single event candidate
    if (!matchedEvent && candidateEvents.length === 1) {
      ...
      const isPrincipal = (evPass && evPass === cleanPasscode) || (syntheticPass && syntheticPass === cleanPasscode);
      const rawGates = Array.isArray(singleEv.checkin_gates)
        ? singleEv.checkin_gates
        : (singleEv.portal_settings?.checkin_gates && Array.isArray(singleEv.portal_settings.checkin_gates) ? singleEv.portal_settings.checkin_gates : []);
      const customGateMatch = rawGates.find(g => (g.passcode || '').trim().toUpperCase() === cleanPasscode);

      if (isPrincipal) {
        matchedGate = { id: "principal", name: "Principal Gate", passcode: evPass || syntheticPass, isPrincipal: true };
      } else if (customGateMatch) {
        matchedGate = { id: customGateMatch.id || `gate_${customGateMatch.passcode}`, name: customGateMatch.name || "Custom Gate", passcode: customGateMatch.passcode, isPrincipal: false };
      }
    ...
    if (!matchedEvent || !matchedStaff) {
      return { 
        success: false, 
        error: "Access Denied: Invalid passcode or email address is not registered as authorized staff for this event." 
      };
    }
    ...
    const sessionPayload = {
      ...
      gateId: matchedGate?.id || "principal",
      gateName: matchedGate?.name || "Principal Gate",
    ...
    return {
      ...
      event: matchedEvent,
      staff: matchedStaff,
      gate: matchedGate || { id: "principal", name: "Principal Gate", passcode: cleanPasscode, isPrincipal: true },
...
export async function fetchCheckinAttendees(eventId = _activeEventId) {
  ...
  try {
    ...
    if (typeof window !== "undefined" && combined.length > 0) {
      try {
        localStorage.setItem(`eventzone_cache_attendees_${resolvedId}`, JSON.stringify(combined));
      } catch {}
    } else if (typeof window !== "undefined" && combined.length === 0) {
      try {
        const cached = localStorage.getItem(`eventzone_cache_attendees_${resolvedId}`);
        if (cached) {
          const parsed = JSON.parse(cached);
          if (Array.isArray(parsed) && parsed.length > 0) return parsed;
        }
...
export async function toggleAttendeeCheckin({ eventId = _activeEventId, attendeeId, checkedIn = true, checkedInBy = '', gateName = '', checkinGate = '' }) {
  if (!attendeeId) return { success: false, error: "Missing attendee ID" };
  const targetId = eventId || _activeEventId;
  const now = checkedIn ? new Date().toISOString() : null;
  const effectiveGate = checkedIn ? (checkinGate || gateName || 'Principal Gate') : null;
  const baseStaff = checkedIn ? (checkedInBy || 'Staff') : null;
  const formattedCheckedInBy = baseStaff ? (effectiveGate ? `${baseStaff} (${effectiveGate})` : baseStaff) : null;

  try {
    const updatePayload = {
      checked_in: !!checkedIn,
      checked_in_at: now,
      checked_in_by: formattedCheckedInBy,
    ...
    if (error) {
      console.warn("toggleAttendeeCheckin participants DB notice (retrying core columns):", error.message);
    ...
    try {
      if (isValidUuid(attendeeId)) {
        ...
        let rsvpQuery = supabase.from('rsvps').update(rsvpPayload).eq('id', attendeeId);
        if (isValidUuid(targetId)) {
          rsvpQuery = rsvpQuery.eq('event_id', targetId);
        }
        const { error: rErr } = await rsvpQuery;
        if (rErr) {
          await supabase.from('rsvps').update({ checked_in: !!checkedIn, checked_in_at: now, checked_in_by: formattedCheckedInBy }).eq('id', attendeeId);
        }
      }
    } catch (rsvpErr) {
    ...
    const mapped = updatedRow ? mapAttendeeFromDb(updatedRow) : {
      id: attendeeId,
      checkedIn: !!checkedIn,
      checked_in: !!checkedIn,
      checkedInAt: now,
      checked_in_at: now,
      checkedInBy: formattedCheckedInBy,
      checked_in_by: formattedCheckedInBy,
...
export async function performQrCheckin({ eventId = _activeEventId, rawPayload, checkedInBy = '', gateName = '', checkinGate = '' }) {
  if (!rawPayload) {
  ...
  try {
    ...
    const isBadgePrintOnly = mappedAttendee.checkedInBy === 'Badge Print' || matchedRow.checked_in_by === 'Badge Print';
    if (mappedAttendee.checkedIn && !isBadgePrintOnly) {
      return {
    ...
    // 5. Check in attendee with Gate attribution
    const checkinTime = new Date().toISOString();
    const targetTable = isRsvpTable ? 'rsvps' : 'participants';
    const effectiveGate = checkinGate || gateName || 'Principal Gate';
    const baseStaff = checkedInBy || 'Gate Staff';
    const formattedCheckedInBy = effectiveGate ? `${baseStaff} (${effectiveGate})` : baseStaff;
    ...
    if (updateErr) {
      console.warn("performQrCheckin update warning (retrying core columns):", updateErr.message);
      const fallbackPayload = {
        checked_in: true,
        checked_in_at: checkinTime,
        checked_in_by: formattedCheckedInBy,
      };
      if (!isRsvpTable) fallbackPayload.status_participation = 'checked_in';
      await supabase.from(targetTable).update(fallbackPayload).eq('id', matchedRow.id);
    }
    ...
    mappedAttendee.checkedInBy = formattedCheckedInBy;
    mappedAttendee.checked_in_by = formattedCheckedInBy;
    mappedAttendee.checkinGate = effectiveGate;
    mappedAttendee.checkin_gate = effectiveGate;
...
export async function updateEventCheckinGates(eventId = _activeEventId, gates = []) {
  const targetId = eventId || _activeEventId;
  if (!isValidUuid(targetId)) return { success: false, error: "Invalid event ID" };
  const cleanGates = Array.isArray(gates) ? gates : [];

  try {
    const { data: ev } = await supabase.from('events').select('portal_settings').eq('id', targetId).maybeSingle();
    const portalSettings = (ev?.portal_settings && typeof ev.portal_settings === 'object') ? ev.portal_settings : {};

    let { error } = await supabase
      .from('events')
      .update({
        checkin_gates: cleanGates,
        portal_settings: {
          ...portalSettings,
          checkin_gates: cleanGates
        }
    ...
    if (error) {
      console.warn("updateEventCheckinGates warning, updating portal_settings fallback:", error.message);
    ...
    return { success: true, gates: cleanGates };
  } catch (e) {
    console.error("updateEventCheckinGates error:", e);
    return { success: false, error: e.message };
  }
}
...
export async function fetchAttendeeConnections(userOrId, eventId) {
  if (!userOrId) {
    const empty = [];
    empty.connections = [];
    empty.pendingSent = [];
    empty.pendingReceived = [];
    return empty;
  }

  const targetEventId = eventId || _activeEventId;
  const userEmail = typeof userOrId === 'object' ? (userOrId.email || '') : (userOrId.includes?.('@') ? userOrId : '');
  const userId = typeof userOrId === 'object' ? (userOrId.id || '') : (!userOrId.includes?.('@') ? userOrId : '');
  const storageKey = `eventzone_connections_${userEmail || userId}_${targetEventId}`;

  try {
    const res = await fetch(`/api/events/${targetEventId}/connections?email=${encodeURIComponent(userEmail)}&userId=${encodeURIComponent(userId)}&_t=${Date.now()}`, {
      cache: "no-store",
      headers: {
    ...
    if (res.ok) {
      ...
      const pendingSent = Array.isArray(data.pendingSent) ? data.pendingSent : [];
      const pendingReceived = Array.isArray(data.pendingReceived) ? data.pendingReceived : [];

      // Return array of accepted connections decorated with pending lists for total backward compatibility
      conns.connections = conns;
      conns.pendingSent = pendingSent;
      conns.pendingReceived = pendingReceived;

      safeLocalStorageSet(storageKey, { connections: conns, pendingSent, pendingReceived });
      return conns;
    }
  } catch (err) {
    console.warn("fetchAttendeeConnections API notice, falling back to cache/Supabase:", err);
  }

  // Fallback from localStorage
  const cached = safeLocalStorageGet(storageKey, null);
  if (cached && typeof cached === 'object') {
    const conns = Array.isArray(cached.connections) ? cached.connections : (Array.isArray(cached) ? cached : []);
    conns.connections = conns;
    conns.pendingSent = Array.isArray(cached.pendingSent) ? cached.pendingSent : [];
    conns.pendingReceived = Array.isArray(cached.pendingReceived) ? cached.pendingReceived : [];
    return conns;
  }

  const fallback = [];
  fallback.connections = [];
  fallback.pendingSent = [];
  fallback.pendingReceived = [];
  return fallback;
}

export async function sendAttendeeConnectionRequest(senderUser, receiverAttendee, eventId, message = "") {
  if (!senderUser || !receiverAttendee) return null;
  const targetEventId = eventId || _activeEventId;
  const senderEmail = senderUser.email || '';
  const recipientEmail = receiverAttendee.email || '';

  try {
    const res = await fetch(`/api/events/${targetEventId}/connections`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        action: "send",
        sender: {
          ...
          email: senderEmail,
          name: senderUser.fullName || senderUser.name || 'Delegate',
          avatar: senderUser.avatar || senderUser.image || '',
          company: senderUser.companyName || senderUser.company || '',
          jobTitle: senderUser.jobTitle || senderUser.title || ''
        },
        recipient: {
          id: receiverAttendee.id || null,
          email: recipientEmail,
          name: receiverAttendee.name || `${receiverAttendee.firstName || ''} ${receiverAttendee.lastName || ''}`.trim() || 'Delegate',
          avatar: receiverAttendee.avatar || receiverAttendee.image || '',
          company: receiverAttendee.company || '',
          jobTitle: receiverAttendee.jobTitle || receiverAttendee.title || ''
        },
        notes: message || ''
      })
    });
    ...
    console.warn("sendAttendeeConnectionRequest API notice:", err);
  }

  // Fallback local creation
  const newConnId = (typeof crypto !== 'undefined' && crypto.randomUUID)
    ? crypto.randomUUID()
...
export async function acceptAttendeeConnectionRequest(connectionId, eventId) {
  ...
  try {
    ...
    console.warn("acceptAttendeeConnectionRequest API error:", err);
  }
  return false;
}

export async function declineAttendeeConnectionRequest(connectionId, eventId) {
  if (!connectionId) return false;
  const targetEventId = eventId || _activeEventId;
  ...
  try {
    ...
    console.warn("declineAttendeeConnectionRequest API error:", err);
  }
  return false;
}

export async function removeAttendeeConnection(connectionId, eventId, partnerEmail = null) {
  if (!connectionId && !partnerEmail) return false;
  const targetEventId = eventId || _activeEventId;
  ...
  try {
    ...
    console.warn("removeAttendeeConnection API error:", err);
  }
  return false;
}

export const cancelAttendeeConnectionRequest = removeAttendeeConnection;

// ─────────────────────────────────────────────
// 📅 ATTENDEE 1-ON-1 MEETINGS
// ─────────────────────────────────────────────

export async function fetchAttendeeMeetings(userOrId, eventId) {
  if (!userOrId) {
    return { meetings: [], upcoming: [], pending: [], past: [] };
  }
  ...
  try {
    ...
    const res = await fetch(`/api/events/${targetEventId}/meetings?${q.toString()}`, {
      cache: "no-store",
    ...
    console.warn("fetchAttendeeMeetings API error:", err);
  }

  return { meetings: [], upcoming: [], pending: [], past: [] };
}

export async function bookAttendeeMeeting(senderUser, recipientAttendee, eventId, meetingData) {
  if (!senderUser || !recipientAttendee) {
  ...
  try {
    const res = await fetch(`/api/events/${targetEventId}/meetings`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        action: "book",
        sender: {
          ...
          name: senderUser.fullName || senderUser.full_name || senderUser.name || "Delegate"
        },
        recipient: {
          id: recipientAttendee.id || recipientAttendee.partnerId || recipientAttendee.connected_user_id,
          email: recipientAttendee.email,
          name: recipientAttendee.fullName || recipientAttendee.name || `${recipientAttendee.firstName || ""} ${recipientAttendee.lastName || ""}`.trim() || "Delegate"
        },
    ...
    const data = await res.json();
    if (!res.ok) {
      return { error: data.error || "Failed to book meeting." };
    }

    return { success: true, meeting: data.meeting };
  } catch (err) {
    console.error("bookAttendeeMeeting error:", err);
    return { error: err.message || "Network error while booking meeting." };
  }
}

export async function acceptAttendeeMeeting(meetingId, eventId) {
  ...
  try {
    const res = await fetch(`/api/events/${targetEventId}/meetings`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: "accept", meetingId })
    });
    return res.ok;
  } catch (err) {
    console.warn("acceptAttendeeMeeting error:", err);
    return false;
  }
}

export async function declineAttendeeMeeting(meetingId, eventId) {
  if (!meetingId) return false;
  const targetEventId = eventId || _activeEventId;
  ...
  try {
    const res = await fetch(`/api/events/${targetEventId}/meetings`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: "decline", meetingId })
    });
    return res.ok;
  } catch (err) {
    console.warn("declineAttendeeMeeting error:", err);
    return false;
  }
}

export async function cancelAttendeeMeeting(meetingId, eventId) {
  if (!meetingId) return false;
  const targetEventId = eventId || _activeEventId;
  ...
  try {
    ...
    console.warn("cancelAttendeeMeeting error:", err);
    return false;
  }
}

export async function fetchMeetingBookedSlots(eventId, date, userIds = []) {
  if (!date) return [];
  const targetEventId = eventId || _activeEventId;
  ...
  try {
    ...
    q.set("date", date);
    if (userIds.length > 0) {
      q.set("userIds", userIds.filter(Boolean).join(","));
    }

    const res = await fetch(`/api/events/${targetEventId}/meetings/booked-slots?${q.toString()}`, {
      cache: "no-store"
    });
    ...
    console.warn("fetchMeetingBookedSlots error:", err);
  }
  return [];
}
...
export async function fetchEventChatMessages(userOrEmail, eventId) {
  if (!userOrEmail) return [];
  const targetEventId = eventId || _activeEventId;
  const userEmail = typeof userOrEmail === 'object' ? (userOrEmail.email || '') : (userOrEmail.includes?.('@') ? userOrEmail : '');
  const userId = typeof userOrEmail === 'object' ? (userOrEmail.id || '') : (!userOrEmail.includes?.('@') ? userOrEmail : '');
  const storageKey = `eventzone_chat_${userEmail || userId}_${targetEventId}`;

  // 1. Fetch from server API
  try {
    const res = await fetch(`/api/events/${targetEventId}/chat?email=${encodeURIComponent(userEmail)}&userId=${encodeURIComponent(userId)}&_t=${Date.now()}`, {
      cache: "no-store",
      headers: {
        "Cache-Control": "no-cache, no-store, must-revalidate",
        "Pragma": "no-cache"
    ...
    if (res.ok) {
      ...
      if (Array.isArray(data.messages)) {
        safeLocalStorageSet(storageKey, data.messages);
        return data.messages;
      }
    }
  } catch (err) {
    console.warn("fetchEventChatMessages API notice, falling back to cache:", err);
  }
...
export async function sendEventChatMessage(senderUser, recipientAttendee, messageText, eventId) {
  ...
  const targetEventId = eventId || _activeEventId;
  const senderEmail = senderUser.email || '';
  const recipientEmail = recipientAttendee.email || '';
  const storageKeySender = `eventzone_chat_${senderEmail || senderUser.id}_${targetEventId}`;

  // 1. Post to API with connection verification
  try {
    const res = await fetch(`/api/events/${targetEventId}/chat`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        sender: {
        ...
        recipient: {
          id: recipientAttendee.id || null,
          email: recipientEmail,
          name: recipientAttendee.name || `${recipientAttendee.firstName || ''} ${recipientAttendee.lastName || ''}`.trim() || 'Delegate',
          avatar: recipientAttendee.avatar || recipientAttendee.image || ''
        },
    ...
    if (!res.ok) {
      ...
      err.notConnected = data.notConnected || false;
      throw err;
    }

    if (data.message) {
      const current = safeLocalStorageGet(storageKeySender, []);
      safeLocalStorageSet(storageKeySender, [...current, data.message]);
      return data.message;
    }
  } catch (err) {
    if (err.notConnected) throw err;
    console.warn("sendEventChatMessage API error, falling back locally:", err);
  }
  ...
  const msgObj = {
    ...
    recipient_id: recipientAttendee.id || null,
    recipient_email: recipientEmail,
...
export async function fetchAllPlatformOrganizers() {
  try {
    ...
    // Consolidate profiles by email (or keep separate if no email)
    const emailToProfiles = new Map();
    const noEmailProfiles = [];

    (profiles || []).forEach(p => {
      const email = (p.email || '').trim().toLowerCase();
      if (!email) {
        noEmailProfiles.push(p);
      } else {
        if (!emailToProfiles.has(email)) {
          emailToProfiles.set(email, []);
        }
        emailToProfiles.get(email).push(p);
      }
    });

    const consolidated = [];

    for (const [email, siblingList] of emailToProfiles.entries()) {
      const allIds = siblingList.map(s => s.id);
      const totalEvents = allIds.reduce((sum, id) => sum + (eventCountsByOrg[id] || 0), 0);

      // Locate profile that has quotas defined
      const profileWithQuota = siblingList.find(s => {
        const m = s.metadata || {};
        const sc = s.social_links || {};
        return (s.max_events !== undefined && s.max_events !== null) ||
               (m.max_events !== undefined && m.max_events !== null) ||
               (sc.max_events !== undefined && sc.max_events !== null);
      }) || siblingList[0];

      const primary = siblingList.find(s => s.role === 'organizer' || s.role === 'super_admin') || siblingList[0];
      const meta = typeof profileWithQuota.metadata === 'object' && profileWithQuota.metadata !== null ? profileWithQuota.metadata : {};
      const socials = typeof profileWithQuota.social_links === 'object' && profileWithQuota.social_links !== null ? profileWithQuota.social_links : {};

      const maxEvents = profileWithQuota.max_events !== undefined ? profileWithQuota.max_events : (meta.max_events !== undefined ? meta.max_events : (socials.max_events !== undefined ? socials.max_events : null));
      const maxAttendees = profileWithQuota.max_attendees !== undefined ? profileWithQuota.max_attendees : (meta.max_attendees !== undefined ? meta.max_attendees : (socials.max_attendees !== undefined ? socials.max_attendees : null));
      const accountStatus = primary.status || meta.status || socials.status || 'active';

      // Pick richest non-empty profile details
      const fullName = siblingList.find(s => s.full_name || s.fullName)?.full_name || primary.full_name || 'Unnamed Organizer';
      const companyName = siblingList.find(s => s.company_name || s.company)?.company_name || primary.company_name || '';
      const jobTitle = siblingList.find(s => s.job_title)?.job_title || primary.job_title || '';
      const phone = siblingList.find(s => s.phone)?.phone || primary.phone || '';
      const location = siblingList.find(s => s.location)?.location || primary.location || '';
      const role = siblingList.some(s => s.role === 'super_admin') ? 'super_admin' : (siblingList.some(s => s.role === 'organizer') ? 'organizer' : (primary.role || 'organizer'));
      const avatarUrl = siblingList.find(s => s.avatar_url)?.avatar_url || primary.avatar_url || '';

      // Resolve the earliest account creation date among sibling profiles
      const validTimestamps = siblingList
        .map(s => s.created_at)
        .filter(Boolean)
        .map(d => new Date(d).getTime())
        .filter(t => !isNaN(t));
      const earliestTimestamp = validTimestamps.length > 0 ? Math.min(...validTimestamps) : null;
      const joinedAt = earliestTimestamp ? new Date(earliestTimestamp).toISOString() : (primary.created_at || '');
 ...
 * Also synchronizes quotas across all sibling profile records sharing the same email.
 */
export async function updateOrganizerQuotas(organizerId, { maxEvents, maxAttendees, status, role }) {
  ...
  try {
    ...
    // Platform Super Admin roles can NEVER be assigned via client updates.
    // Super admin privileges are strictly provisioned directly in the database.
    ...
    // Sibling profile synchronization: sync across all profiles sharing this email
    if (cur?.email && typeof cur.email === 'string' && cur.email.trim()) {
      try {
        await supabase
          .from('profiles')
          .update(payload)
          .ilike('email', cur.email.trim());
      } catch (sibErr) {
        console.warn("Sibling profiles quota sync warning:", sibErr);
      }
    }
...
export async function updateEventHeroFeatured(eventId, { isHeroFeatured, heroOrder }) {
  ...
  try {
    const { data: currentEvent, error: fetchErr } = await supabase
      .from('events')
      .select('status, portal_settings')
      .eq('id', eventId)
      .maybeSingle();

    if (fetchErr) throw fetchErr;

    if (Boolean(isHeroFeatured) && currentEvent?.status !== 'published') {
      return { success: false, error: "Cannot feature a non-published or suspended event on the Homepage Hero." };
    }
 ...
 * If suspended or un-published, automatically unpins the event from the Homepage Hero.
 */
...
/**
 * Fetches all newsletter subscribers for the platform super admin.
 */
export async function fetchAllNewsletterSubscribers() {
  try {
    ...
    console.error("fetchAllNewsletterSubscribers error:", err);
    return [];
  }
}
 ...
 */
export async function subscribeToNewsletter(email, source = 'footer_newsletter') {
  const cleanEmail = (email || '').trim().toLowerCase();
...
/**
 * Deletes a newsletter subscriber by ID (Super Admin only).
 */
...
/**
 * Generates a clean, unique reference code for quote requests (e.g., EZQ-8F3A29).
 */
export function generateQuoteReferenceCode() {
...
/**
 * Submits a new quote request to public.quote_requests.
 */
export async function submitQuoteRequest(quoteData) {
  try {
    const referenceCode = quoteData.reference_code || generateQuoteReferenceCode();
    const payload = {
      reference_code: referenceCode,
      full_name: (quoteData.full_name || quoteData.fullName || "").trim(),
      email: (quoteData.email || "").trim().toLowerCase(),
      phone: (quoteData.phone || "").trim(),
      company_name: (quoteData.company_name || quoteData.companyName || "").trim(),
      job_title: (quoteData.job_title || quoteData.jobTitle || "").trim(),
      city_wilaya: quoteData.city_wilaya || quoteData.cityWilaya || quoteData.wilaya || "",
      event_name: (quoteData.event_name || quoteData.eventName || "").trim(),
      event_type: quoteData.event_type || quoteData.eventType || "",
      attendees_count: quoteData.attendees_count || quoteData.attendeesCount || "",
      event_date: quoteData.event_date || quoteData.eventDate || "",
      duration: quoteData.duration || "",
      venue_status: quoteData.venue_status || quoteData.venueStatus || "",
      services: Array.isArray(quoteData.services) ? quoteData.services : [],
      budget_range: quoteData.budget_range || quoteData.budgetRange || "",
      additional_details: (quoteData.additional_details || quoteData.additionalDetails || quoteData.notes || "").trim(),
...
/**
 * Fetches all quote requests for the platform super admin back office.
 */
export async function fetchAllQuoteRequests() {
  try {
    const { data, error } = await supabase
      .from("quote_requests")
      .select("*")
    ...
    console.error("fetchAllQuoteRequests error:", err);
    return [];
  }
}

/**
 * Updates a quote request status, admin internal notes, or quoted amount (Super Admin only).
 */
export async function updateQuoteRequest(quoteId, updates = {}) {
  ...
  try {
    ...
    console.error("updateQuoteRequest error:", err);
    return { success: false, error: err.message || "Failed to update quote request" };
  }
}

/**
 * Deletes a quote request by ID (Super Admin only).
 */
export async function deleteQuoteRequest(quoteId) {
  ...
  try {
    ...
    console.error("deleteQuoteRequest error:", err);
    return false;
  }
}

// ─────────────────────────────────────────────
//  INVOICING & QUOTES DATA LAYER
// ─────────────────────────────────────────────

/**
 * Fetches all invoicing profiles for a user or returns a sensible default if none exist
 */
export async function fetchInvoicingProfiles(userId) {
  try {
    let query = supabase
      .from('invoicing_profiles')
      .select('*')
    ...
    const { data, error } = await query;
    if (error) {
      console.warn("fetchInvoicingProfiles error:", error.message);
      const local = safeLocalStorageGet(`eventzone_invoicing_profiles_${userId || 'default'}`, []);
      return local.length ? local : [];
    }

    if (typeof window !== 'undefined') {
      safeLocalStorageSet(`eventzone_invoicing_profiles_${userId || 'default'}`, data || []);
    }
    return data || [];
  } catch (err) {
    console.warn("fetchInvoicingProfiles fallback error:", err);
    return safeLocalStorageGet(`eventzone_invoicing_profiles_${userId || 'default'}`, []);
  }
}

/**
 * Creates or updates an invoicing profile
 */
export async function upsertInvoicingProfile(profileData = {}) {
  try {
    const payload = {
      ...profileData,
      updated_at: new Date().toISOString()
    ...
    if (typeof window !== 'undefined' && result) {
      const key = `eventzone_invoicing_profiles_${result.user_id || 'default'}`;
      const existing = safeLocalStorageGet(key, []);
      const idx = existing.findIndex(p => p.id === result.id);
      const updated = idx >= 0 ? existing.map(p => p.id === result.id ? result : p) : [result, ...existing];
      safeLocalStorageSet(key, updated);
    }
    return result;
  } catch (err) {
    console.error("upsertInvoicingProfile error:", err);
    // Offline / demo fallback
    const fallbackProfile = {
      id: profileData.id || `local-prof-${Date.now()}`,
      ...profileData,
      updated_at: new Date().toISOString()
    };
    if (typeof window !== 'undefined') {
      const key = `eventzone_invoicing_profiles_${fallbackProfile.user_id || 'default'}`;
      const existing = safeLocalStorageGet(key, []);
      const idx = existing.findIndex(p => p.id === fallbackProfile.id);
      const updated = idx >= 0 ? existing.map(p => p.id === fallbackProfile.id ? fallbackProfile : p) : [fallbackProfile, ...existing];
      safeLocalStorageSet(key, updated);
    }
...
/**
 * Deletes an invoicing profile
 */
export async function deleteInvoicingProfile(profileId, userId) {
  ...
  try {
    ...
    if (error) throw error;
    if (typeof window !== 'undefined') {
      const key = `eventzone_invoicing_profiles_${userId || 'default'}`;
      const existing = safeLocalStorageGet(key, []);
      safeLocalStorageSet(key, existing.filter(p => p.id !== profileId));
    }
    return true;
  } catch (err) {
    console.error("deleteInvoicingProfile error:", err);
    return false;
  }
}

/**
 * Fetches all invoices, quotes, and proformas for an organizer or event
 */
export async function fetchInvoices({ userId = null, eventId = null, profileId = null } = {}) {
  try {
    let query = supabase
      .from('invoices')
      .select('*')
    ...
    const { data, error } = await query;
    if (error) {
      console.warn("fetchInvoices query error:", error.message);
      const cacheKey = `eventzone_invoices_${userId || 'all'}_${eventId || 'all'}`;
      return safeLocalStorageGet(cacheKey, []);
    }
    ...
    console.warn("fetchInvoices fallback error:", err);
    const cacheKey = `eventzone_invoices_${userId || 'all'}_${eventId || 'all'}`;
    return safeLocalStorageGet(cacheKey, []);
  }
}
...
export async function fetchInvoiceById(invoiceId) {
  ...
  try {
    ...
    console.warn("fetchInvoiceById error:", err);
    return null;
  }
}

/**
 * Fetches a single invoice by its public share token
 */
export async function fetchInvoiceByToken(shareToken) {
  ...
  try {
    ...
    console.warn("fetchInvoiceByToken error:", err);
    return null;
  }
}

/**
 * Creates or updates an invoice/quote document
 */
export async function upsertInvoice(invoiceData = {}) {
  try {
    ...
    // Update local cache
    if (typeof window !== 'undefined' && result) {
      const cacheKey = `eventzone_invoices_${result.user_id || 'all'}_${result.event_id || 'all'}`;
      const existing = safeLocalStorageGet(cacheKey, []);
      const idx = existing.findIndex(i => i.id === result.id);
      const updated = idx >= 0 ? existing.map(i => i.id === result.id ? result : i) : [result, ...existing];
      safeLocalStorageSet(cacheKey, updated);
    }
    return result;
  } catch (err) {
    console.error("upsertInvoice error:", err);
    // Offline / demo fallback
    const fallbackInvoice = {
      id: invoiceData.id || `local-inv-${Date.now()}`,
      share_token: invoiceData.share_token || Math.random().toString(36).substring(2, 18),
      ...invoiceData,
      updated_at: new Date().toISOString()
    };
    if (typeof window !== 'undefined') {
      const cacheKey = `eventzone_invoices_${fallbackInvoice.user_id || 'all'}_${fallbackInvoice.event_id || 'all'}`;
      const existing = safeLocalStorageGet(cacheKey, []);
      const idx = existing.findIndex(i => i.id === fallbackInvoice.id);
      const updated = idx >= 0 ? existing.map(i => i.id === fallbackInvoice.id ? fallbackInvoice : i) : [fallbackInvoice, ...existing];
      safeLocalStorageSet(cacheKey, updated);
    }
...
/**
 * Updates status of an invoice (e.g. from row dropdown: brouillon, envoye, encaisse, etc.)
 */
export async function updateInvoiceStatus(invoiceId, newStatus) {
  if (!invoiceId || !newStatus) return false;
  try {
    const { data, error } = await supabase
      .from('invoices')
      .update({
...
/**
 * Convert a Devis (Quote) or Proforma into a Facture (Invoice)
 */
export async function convertQuoteToInvoice(quoteId, newNumber = null) {
  if (!quoteId) return null;
  try {
    const original = await fetchInvoiceById(quoteId);
    if (!original) return null;

    const payload = {
      ...original,
      document_type: 'facture',
      document_number: newNumber || `EZ-26-${String(Math.floor(Math.random() * 9000) + 1000)}`,
      status: 'brouillon',
    ...
    delete payload.id;
    delete payload.share_token;

    return await upsertInvoice(payload);
  } catch (err) {
    console.error("convertQuoteToInvoice error:", err);
    return null;
  }
}
 ...
 */
export async function duplicateInvoice(invoiceId, newNumber = null) {
  if (!invoiceId) return null;
  try {
    ...
    const payload = {
      ...original,
      document_number: newNumber || `${original.document_number || 'DOC'}-COPIE`,
      status: 'brouillon',
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString()
    };
...
/**
 * Fetches saved clients for quick autofill
 */
export async function fetchInvoicingClients(userId) {
  try {
    let query = supabase
    ...
    const { data, error } = await query;
    if (error) {
      console.warn("fetchInvoicingClients error:", error.message);
      return safeLocalStorageGet(`eventzone_invoicing_clients_${userId || 'default'}`, []);
    }
    return data || [];
  } catch (err) {
    console.warn("fetchInvoicingClients fallback error:", err);
    return safeLocalStorageGet(`eventzone_invoicing_clients_${userId || 'default'}`, []);
  }
}

/**
 * Saves a client to the address book
 */
export async function upsertInvoicingClient(clientData = {}) {
  try {
    const payload = {
      ...clientData,
      updated_at: new Date().toISOString()
    ...
    console.error("upsertInvoicingClient error:", err);
    return clientData;
  }
}

// ─────────────────────────────────────────────
//  MOBILE APP OPERATIONS & SUBSCRIPTION ADMIN
// ─────────────────────────────────────────────

/**
 * Fetches all app users from `profiles` with computed subscription and trial metrics.
 */
export async function fetchAllAppUsersAdmin() {
  try {
    ...
    const enriched = (profiles || []).map(p => {
      ...
      const subEndDate = p.subscription_end_date ? new Date(p.subscription_end_date) : null;
      ...
      if (subEndDate && subEndDate > now) {
        ...
        effectiveEndDate = subEndDate.toISOString();
      } else if (trialEndDate > now && (!subEndDate || subEndDate <= trialEndDate)) {
        subscriptionStatus = 'trial';
        daysRemaining = Math.max(0, Math.ceil((trialEndDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24)));
        effectiveEndDate = trialEndDate.toISOString();
      } else {
        subscriptionStatus = 'expired';
        daysRemaining = 0;
        effectiveEndDate = subEndDate ? subEndDate.toISOString() : trialEndDate.toISOString();
      }
    ...
    console.error("fetchAllAppUsersAdmin error:", err);
    return [];
  }
}

/**
 * Updates a user's subscription end date and synchronizes the `subscriptions` table.
 */
export async function updateUserProfileSubscription(userId, { daysToAdd, customEndDate, tier = 'Pro' } = {}) {
  try {
    ...
    if (customEndDate !== undefined) {
      newEndDate = customEndDate ? new Date(customEndDate).toISOString() : null;
      ...
      if (currentProfile?.subscription_end_date) {
        const existingEnd = new Date(currentProfile.subscription_end_date);
    ...
    const isSubActive = newEndDate && new Date(newEndDate) > new Date();
    ...
    console.error("updateUserProfileSubscription error:", err);
    return { success: false, error: err.message || "Failed to update subscription" };
  }
}

/**
 * Fetches all promo codes for the mobile app.
 */
export async function fetchAllPromoCodesAdmin() {
  try {
    const { data, error } = await supabase
      .from('promo_codes')
      .select('*')
    ...
    console.error("fetchAllPromoCodesAdmin error:", err);
    return [];
  }
}
 ...
 */
export async function createPromoCodeAdmin({ code, discountPercentage }) {
  try {
    if (!code || !code.trim()) throw new Error("Promo code string is required");
    ...
    console.error("createPromoCodeAdmin error:", err);
    return { success: false, error: err.message || "Failed to create promo code" };
  }
}

/**
 * Toggles active state of a promo code.
 */
export async function togglePromoCodeStatusAdmin(id, currentStatus) {
  try {
    const { data, error } = await supabase
      .from('promo_codes')
    ...
    console.error("togglePromoCodeStatusAdmin error:", err);
    return { success: false, error: err.message || "Failed to toggle promo code" };
  }
}
...
export async function deletePromoCodeAdmin(id) {
  try {
    ...
    console.error("deletePromoCodeAdmin error:", err);
    return { success: false, error: err.message || "Failed to delete promo code" };
  }
}

/**
 * Sends a custom push notification to app users via the `send-notification` Supabase Edge Function.
 * @param {Object} options
 * @param {string} options.target - 'All Users' | 'Paid Users' | 'Free Users' | specific user_id
 * @param {string} options.title - Notification title
 * @param {string} options.message - Notification body/message
 */
export async function sendCustomPushNotification({ target, title, message }) {
  try {
    if (!title?.trim() || !message?.trim() || !target) {
      throw new Error("Target, title, and message are required.");
    }
    ...
    return {
      success: true,
      message: data?.message || "Push notification dispatched successfully.",
      data,
    };
  } catch (err) {
    console.error("sendCustomPushNotification error:", err);
    return {
      success: false,
