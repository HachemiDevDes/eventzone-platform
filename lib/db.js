    ...
    try {
      ...
      if (eventIds.length > 0) {
        ...
        const eventsList = await cachedQuery(`events_summary_${sortedIds}`, async () => {
          const { data } = await supabase
            .from('events')
            .select('id, name, location, start_date, end_date, badge_url, badge_settings')
            .in('id', eventIds);
          return data || [];
        }, 60000);
      ...
      return participants.map(row => {
        ...
        return {
          ...
          templateUrl: row.template_url || row.badge_url || ev.badge_url || "",
          badgeUrl: row.badge_url || ev.badge_url || "",
          badgeSettings: row.badge_settings || ev.badge_settings || {},
        };
      });
    } catch (err) {
