...
import { SUPER_ADMIN_EMAILS, isPlatformSuperAdminEmail } from "./constants";
...
export async function verifyOrganizerSession(request, eventId) {
  try {
    ...
    const isLocalhost = hostHeader.includes("localhost") || hostHeader.includes("127.0.0.1") || process.env.NODE_ENV === "development";
    ...
    if (token) {
      try {
        ...
        console.warn("verifyOrganizerSession token lookup error:", tokenErr);
      }
    }

    // If no valid user from token, check local dev fallback
    if (!user) {
      if (isLocalhost) {
        user = { id: "dev-local-user", email: "dev@eventzone.pro" };
      } else {
    ...
    if (user?.email && (isPlatformSuperAdminEmail(user.email) || SUPER_ADMIN_EMAILS.some(e => e.toLowerCase() === user.email.toLowerCase()))) {
    ...
    if (eventId) {
      ...
      let query = supabase
        .from("events")
        .select("id, organizer_id, contact_email");
      ...
      if (evErr || !ev) {
        if (isLocalhost) {
          return { authorized: true, user, isDevFallback: true };
        }
        return { authorized: false, error: "Event not found.", status: 404 };
      }

      if (ev.organizer_id && ev.organizer_id !== user.id) {
        // Check if user email matches contact_email
        if (ev.contact_email && user.email && ev.contact_email.toLowerCase() === user.email.toLowerCase()) {
 ...
 */
export async function verifyCheckinStaffOrOrganizer(request, eventId, fallbackCredentials = {}) {
  if (!eventId) {
    return { authorized: false, error: "Event ID is required.", status: 400 };
  }

  // Safely extract URL search params if present
  let urlParams = null;
  try {
    if (request?.url) {
  ...
  // 3. Check Gate Staff passcode headers / query / fallback
  const passcode = (
    request.headers.get("x-checkin-passcode") ||
    request.headers.get("x-passcode") ||
    fallbackCredentials.passcode ||
    urlParams?.get("passcode") ||
    urlParams?.get("checkin_passcode") ||
    ""
  ).trim().toUpperCase();
  ...
  const staffEmail = (
    ...
    fallbackCredentials.staffEmail ||
    fallbackCredentials.email ||
    urlParams?.get("staffEmail") ||
    urlParams?.get("email") ||
    ""
  ).trim().toLowerCase();
  ...
  if (passcode) {
    ...
    let query = supabase
      .from("events")
      .select("id, organizer_id, contact_email, checkin_passcode, checkin_gates, portal_settings");
    ...
    let { data: ev, error: evErr } = await query.maybeSingle();

    if (evErr) {
      // Fallback query without checkin_gates column in case migration is pending
      let fallbackQuery = supabase
        .from("events")
        .select("id, organizer_id, contact_email, checkin_passcode, portal_settings");
      if (/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(eventId)) {
        fallbackQuery = fallbackQuery.eq("id", eventId);
      } else {
        fallbackQuery = fallbackQuery.or(`id.eq.${eventId},slug.eq.${eventId}`);
      }
    ...
    const isPrincipal = (evPass && evPass === passcode) || (syntheticPass && syntheticPass === passcode);

    const rawGates = Array.isArray(ev.checkin_gates)
      ? ev.checkin_gates
      : (ev.portal_settings?.checkin_gates && Array.isArray(ev.portal_settings.checkin_gates) ? ev.portal_settings.checkin_gates : []);

    const customGateMatch = rawGates.find(g => (g.passcode || '').trim().toUpperCase() === passcode);
    ...
    const isPassValid = Boolean(matchedGate);
    ...
    if (isPassValid) {
      ...
      if (staffEmail) {
        ...
        if (tm) {
          return { authorized: true, isStaff: true, gate: matchedGate, staff: { name: tm.name, email: staffEmail, role: tm.role } };
        }
        ...
        if (prof && prof.id === ev.organizer_id) {
          return { authorized: true, isOrganizer: true, gate: matchedGate, staff: { name: prof.full_name, email: staffEmail, role: "Organizer" } };
        }

        const hostEmail = (ev.contact_email || "").trim().toLowerCase();
        if (hostEmail && hostEmail === staffEmail) {
          return { authorized: true, isOrganizer: true, gate: matchedGate, staff: { name: "Organizer", email: staffEmail, role: "Organizer" } };
        }

        // If exact passcode is verified, allow Gate Staff role
        return { authorized: true, isStaff: true, gate: matchedGate, staff: { name: staffEmail.split('@')[0], email: staffEmail, role: "Gate Staff" } };
      }

      // Passcode is valid, authorized as Gate Terminal
      return { authorized: true, isStaff: true, gate: matchedGate, staff: { name: "Gate Staff", role: "Gate Staff" } };
    }
  }
