import { isPlatformSuperAdminEmail } from "./constants.js";
...
export const EVENT_MODULES = [
  ...
  },
  {
    ...
    description: "Invoices, quotations, proforma billing, tax rates, and client balances",
    category: "Commercial",
    icon: "Receipt"
  },
  {
  ...
  },
  {
    ...
    description: "Public attendee directory, badge preview, session registration, and 1-on-1 chats",
    category: "People",
    icon: "Globe"
  },
  {
  ...
  },
  {
    ...
    description: "Equipment inventory, specific exhibitor equipment, suppliers, VIP travel, checklists, run-of-show",
    category: "Operations",
    icon: "Boxes"
  },
  {
...
export const ROLE_PRESETS = [
  ...
  },
  {
    ...
    description: "Editor for Logistics, Specific Equipment, Floor Plans, and Exhibitors; Viewer for Agenda & Attendees.",
...
export function getEffectivePermissions(
  ...
  if (simulatedMemberId) {
    ...
    if (simMember) {
      ...
      const isSimAdmin = roleLower === "admin" || roleLower === "owner" || roleLower === "co-host";
      return {
        hasAccess: true,
        isOwner: false,
        isAdmin: isSimAdmin,
  ...
  // 2. If no user is logged in: No organizer dashboard access
  if (!currentUser) {
    return {
      hasAccess: false,
      isOwner: false,
      ...
      isSimulated: false,
      isNonMember: true,
      permissions: Object.fromEntries(EVENT_MODULES.map(m => [m.id, "none"]))
    };
  }
  ...
  // 5. Match current event in userEvents (from fetchUserEvents)
  const matchedEventInUserEvents = Array.isArray(userEvents) ? userEvents.find(ev => {
    if (!ev) return false;
    ...
    const evTitle = (ev.title || ev.name || "").toLowerCase().trim();
    const activeTitle = (activeEvent?.title || activeEvent?.name || "").toLowerCase().trim();
    if (evTitle && activeTitle && evTitle === activeTitle) return true;
    return false;
  }) : null;

  // 6. Direct Event Ownership Verification:
  // True owner ONLY if user ID or email matches event creator/organizer/host/contact fields,
  // OR if matched event in userEvents is NOT marked with isTeamMember: true.
  ...
  const isDirectOwnerInUserEvents = !!(matchedEventInUserEvents && !matchedEventInUserEvents.isTeamMember);

  const isDirectOwner = isMatchByUserId || isMatchByEmail || isDirectOwnerInUserEvents;
  ...
  const isExplicitTeamEvent = activeEvent?.isTeamMember === true || matchedEventInUserEvents?.isTeamMember === true;

  if (matchedMemberInTeam || isExplicitTeamEvent) {
    const memberObj = matchedMemberInTeam || {
      id: activeEvent?.teamMemberId || matchedEventInUserEvents?.teamMemberId,
      role: activeEvent?.teamRole || matchedEventInUserEvents?.teamRole || "Staff",
      permissions: activeEvent?.teamPermissions || matchedEventInUserEvents?.teamPermissions || {},
      status: activeEvent?.teamStatus || matchedEventInUserEvents?.teamStatus || "Active"
    };

    const roleLower = (memberObj.role || "").toLowerCase();
    const isMemberAdmin = roleLower === "admin" || roleLower === "owner" || roleLower === "co-host";

    const memberPermissions = isMemberAdmin
      ? Object.fromEntries(EVENT_MODULES.map(m => [m.id, "editor"]))
  ...
  // 8. Explicit Demo Sandbox Summit Context:
  // Only the designated demo sandbox events or when initializing a brand-new blank event without an ID
  const isDemoEvent = currentEventIdStr === "default-summit-2025" || 
    currentEventIdStr === "myevent" ||
    currentEventIdStr === "00000000-0000-0000-0000-000000000001" ||
    (!currentEventIdStr && !activeEvent);

  if (isDemoEvent && isOrganizerUser) {
    return {
      hasAccess: true,
      isOwner: true,
      isAdmin: true,
  ...
  // 9. Default fallback for non-team visitors / unauthorized users: No organizer dashboard access
  return {
    hasAccess: false,
    isOwner: false,
    isAdmin: false,
...
/**
 * Check if the user is authorized to access the event dashboard
 */
export function hasEventAccess(effectivePermissions) {
  if (!effectivePermissions) return false;
  if (effectivePermissions.hasAccess === false) return false;
  return Boolean(
    effectivePermissions.isOwner ||
    effectivePermissions.isAdmin ||
    effectivePermissions.member ||
    effectivePermissions.isSimulated
  );
}
 ...
 */
export function canViewModule(moduleKey, permissionsOrEffective = {}) {
  if (!moduleKey) return false;
  if (permissionsOrEffective?.hasAccess === false) return false;
  if (permissionsOrEffective?.isAdmin || permissionsOrEffective?.isOwner) return true;
  const perms = permissionsOrEffective?.permissions || permissionsOrEffective || {};
 ...
 */
export function canEditModule(moduleKey, permissionsOrEffective = {}) {
  if (!moduleKey) return false;
  if (permissionsOrEffective?.hasAccess === false) return false;
  if (permissionsOrEffective?.isAdmin || permissionsOrEffective?.isOwner) return true;
  const perms = permissionsOrEffective?.permissions || permissionsOrEffective || {};
  return perms[moduleKey] === "editor";
}

/**
 * Check if current permissions restrict a module to read-only viewer mode
 */
export function isModuleReadOnly(moduleKey, permissionsOrEffective = {}) {
  return !canEditModule(moduleKey, permissionsOrEffective);
}

/**
 * Get permission level for a module: 'editor' | 'viewer' | 'none'
 */
export function getModulePermission(moduleKey, permissionsOrEffective = {}) {
  if (!moduleKey) return "none";
  if (permissionsOrEffective?.isAdmin || permissionsOrEffective?.isOwner) return "editor";
  const perms = permissionsOrEffective?.permissions || permissionsOrEffective || {};
