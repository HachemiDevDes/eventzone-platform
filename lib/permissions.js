...
export function getEffectivePermissions(currentUser, activeEvent, team = [], simulatedMemberId = null) {
  ...
  // If no user is logged in: read-only viewer mode
  if (!currentUser) {
    return {
      isOwner: false,
      isAdmin: false,
  ...
  // Check if current user is event owner or global platform admin
  ...
  const eventContactEmail = (activeEvent?.contact_email || activeEvent?.contactEmail || activeEvent?.host_email || activeEvent?.hostEmail || "").toLowerCase().trim();

  const isOwner = !!(
    activeEvent && (
      (eventOrganizerId && String(eventOrganizerId) === String(currentUser.id)) ||
      (eventContactEmail && userEmail && eventContactEmail === userEmail) ||
      (!eventOrganizerId && currentUser.role === "organizer")
    )
  );

  const isGlobalAdmin = currentUser.role === "admin" || currentUser.isAdmin === true;
  ...
  if (matchedMember) {
    const isMemberAdmin = (matchedMember.role || "").toLowerCase() === "admin" || (matchedMember.role || "").toLowerCase() === "organizer" || (matchedMember.role || "").toLowerCase() === "owner";
    return {
  ...
  // If user is a registered organizer and the event has no conflicting owner, grant editor access
  if (isOrganizerUser && !eventOrganizerId) {
    return {
      isOwner: true,
      isAdmin: true,
      isSimulated: false,
  ...
  // Default fallback for unauthorized non-team user: Viewer mode
  return {
    isOwner: false,
    isAdmin: false,
    isSimulated: false,
