...
export function cleanupLocalStorageQuota() {
  ...
  try {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (!k) continue;
      // Strictly protect user auth, language, offline queues, active event ID, and offline data caches
      if (
        k === "eventzone_user" ||
        k === "app_language" ||
        k === "eventzone_active_event_id" ||
        k === "eventzone_visitor_registrations" ||
        k.startsWith("eventzone_visitor_") ||
        k.startsWith("eventzone_offline_queue_") ||
        k.startsWith("eventzone_cache_") ||
        k.startsWith("eventzone_cached_event_") ||
...
export function safeLocalStorageGet(key, fallback = null) {
  ...
  try {
    ...
    try {
      ...
      // If item was stored as a raw string (e.g. UUID, plain text), return the raw string
      return item;
    }
  } catch (err) {
    return fallback;
