...
import { formatCurrency, calculateFiscalStamp } from "./invoicingConstants.js";
...
export async function generateInvoicePdfBuffer(invoice = {}) {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.rect(margin, currentY, contentWidth, 3).fill("#0b5cdb");
      currentY += 20;
      ...
      doc
        ...
        .fontSize(11)
        .fillColor("#64748b")
        .text(invoice.document_number || "EZ-26-0001", rightX, currentY + 22, { width: 220, align: "right" });

      currentY += 48;

      // 3. Metadata Pill Box (Date d'émission, Échéance, Référence)
      const pillBoxHeight = 36;
      doc
        .roundedRect(margin, currentY, contentWidth, pillBoxHeight, 6)
        .fillAndStroke("#f8fafc", "#e2e8f0");

      const col1W = contentWidth / 3;
      const pillTextY = currentY + 7;

      // Date d'émission
      doc.font("Helvetica-Bold").fontSize(7).fillColor("#94a3b8").text("DATE D'ÉMISSION", margin + 15, pillTextY);
      doc.font("Helvetica-Bold").fontSize(9).fillColor("#0f172a").text(invoice.issue_date || new Date().toISOString().split("T")[0], margin + 15, pillTextY + 11);

      // Échéance
      doc.font("Helvetica-Bold").fontSize(7).fillColor("#94a3b8").text("ÉCHÉANCE", margin + col1W + 15, pillTextY);
      doc.font("Helvetica-Bold").fontSize(9).fillColor("#0f172a").text(invoice.due_date || "—", margin + col1W + 15, pillTextY + 11);

      // Référence
      doc.font("Helvetica-Bold").fontSize(7).fillColor("#94a3b8").text("RÉFÉRENCE", margin + col1W * 2 + 15, pillTextY);
      doc.font("Helvetica-Bold").fontSize(9).fillColor("#0f172a").text(invoice.document_number || "—", margin + col1W * 2 + 15, pillTextY + 11);

      currentY += pillBoxHeight + 20;

      // 4. Two Columns: Émetteur (Left) & Destinataire (Right)
      ...
      if (invoice.emitter_email) {
        doc.font("Helvetica").fontSize(8).fillColor("#64748b").text(invoice.emitter_email, margin, emitY);
        emitY += 12;
      }
      ...
      doc.font("Helvetica-Bold").fontSize(11).fillColor("#0f172a").text(invoice.client_name || "Client", clientX, currentY + 14);
