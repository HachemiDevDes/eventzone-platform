...
export async function generateMetadata({ params }) {
  ...
  const title = `Conferences & Events in ${city.name} | Eventzone`;
  const description = `${city.description} Book tickets, explore interactive floor plans, and view schedules on Eventzone.`;
  const canonicalUrl = `https://eventzone.pro/events/city/${city.slug}`;
  ...
  return {
    ...
    keywords: [
      `events in ${city.name}`,
      `conferences in ${city.name}`,
      `summits ${city.name}`,
      `expos ${city.name}`,
      `${city.nameFr} événements`,
      `${city.nameAr} مؤتمرات`,
      "Eventzone",
      "Algeria events"
    ],
...
export default async function CityPage({ params }) {
  ...
  try {
    ...
    console.warn("City page events fetch notice:", err);
  }

  const cityEvents = filterEventsByCity(allEvents, city.slug);
  const otherCities = POPULAR_CITIES.filter(c => c.slug !== city.slug);
  const baseUrl = process.env.NEXT_PUBLIC_APP_URL || "https://eventzone.pro";
  const canonicalUrl = `${baseUrl}/events/city/${city.slug}`;
  ...
  const collectionLd = {
    "@context": "https://schema.org",
    "@type": "CollectionPage",
    "name": `Conferences & Events in ${city.name}`,
  ...
  const itemListLd = cityEvents.length > 0 ? {
    "@context": "https://schema.org",
    "@type": "ItemList",
    "name": `Events in ${city.name}`,
    "itemListElement": cityEvents.map((ev, index) => ({
      "@type": "ListItem",
      "position": index + 1,
      "item": {
        "@type": "Event",
      ...
      <section className="bg-slate-950 text-white py-12 sm:py-16 border-b border-slate-800 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-8 relative z-10 space-y-4">
          ...
          <nav className="flex items-center gap-2 text-xs text-slate-400 font-medium">
            ...
            <span>Cities</span>
            <ChevronRight size={12} />
            <span className="text-blue-400 font-bold">{city.name}</span>
          </nav>
          ...
          <h1 className="text-3xl sm:text-5xl font-black tracking-tight text-white">
            Conferences &amp; Summits in {city.name}
          </h1>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-8 py-10 sm:py-14 space-y-10">
        <div className="flex items-center justify-between border-b border-slate-200 pb-4">
          <h2 className="text-xl sm:text-2xl font-extrabold text-slate-900">
            Upcoming Events in {city.name} ({cityEvents.length})
          </h2>
          <Link
            href="/"
            className="text-xs font-bold text-blue-600 hover:text-blue-700 flex items-center gap-1"
          >
            <span>View All Algerian Events</span>
            <ArrowRight size={14} />
          </Link>
        </div>
        ...
        {cityEvents.length === 0 ? (
          <div className="bg-white border border-dashed border-slate-300 rounded-3xl p-10 text-center flex flex-col items-center justify-center gap-3">
            ...
            <h3 className="text-base font-bold text-slate-900">No events currently scheduled in {city.name}</h3>
            <p className="text-xs text-slate-500 max-w-md">
              Check back soon as organizers frequently publish new conferences, or browse all events across Algeria.
            </p>
            <Link
              href="/"
                  ...
                  <div className="p-6 space-y-3">
                    ...
                    <div className="flex flex-wrap items-center gap-2 pt-3 border-t border-slate-100 text-xs">
                      ...
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-slate-100 text-slate-700 font-semibold">
                        ...
                        <span className="truncate">{ev.location || city.name}</span>
                      </span>
                    </div>
                  </div>
                </div>
        ...
        {/* Explore Other Cities Navigation Section */}
        <section className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 space-y-4">
          <h3 className="text-base sm:text-lg font-bold text-slate-900">
            Explore Events in Other Algerian Cities
          </h3>
          <div className="flex flex-wrap gap-2.5">
            {otherCities.map(other => (
              <Link
                key={other.slug}
                href={`/events/city/${other.slug}`}
              ...
              >
                ...
                <span>{other.name} ({other.wilaya.split(" - ")[0]})</span>
              </Link>
            ))}
          </div>
        </section>
